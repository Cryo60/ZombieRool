...
public class M16A4WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 30;
    private static final int MAX_RESERVE    = MAX_AMMO * 6;
    private static final int COOLDOWN_TICKS = 2; // This is your fire rate, 2 ticks = 0.1 seconds
    private static final int RELOAD_TIME    = 60; // ticks (3s)
    
    private static final float WEAPON_DAMAGE = 9.5f;
    private static final float PAP_BONUS_DAMAGE = 8.0f; 
    
    private static final float BASE_HEADSHOT_DAMAGE = 12.0f; // Dégâts de base du headshot (multiplie la base)
    private static final float PAP_HEADSHOT_BONUS = 8.0f;    // Bonus de dégâts du headshot si PaP

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 3.0f; // Vitesse de base du projectile
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.2f; // Multiplicateur de vitesse pour PaP (ex: 3.0 * 1.2 = 3.6)
    private static final float BASE_PROJECTILE_SPREAD = 0.5f; // Dispersion de base (plus petit = plus précis)
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.5f; // Multiplicateur de dispersion pour PaP (ex: 0.5 * 0.5 = 0.25, donc plus précis)

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 0.7f; // Recul en haut/bas (pitch) pour un tir de base
    private static final float BASE_RECOIL_YAW = 0.3f;   // Recul gauche/droite (yaw) pour un tir de base
    private static final float PAP_RECOIL_MULTIPLIER = 0.6f; // Multiplicateur de recul pour PaP (ex: 0.7 * 0.6 = 0.42, moins de recul)


    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.M16A4_FIRE.get();
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.M16A4_RELOADING.get();
    ...
    public float getHeadshotPapBonusDamage(ItemStack stack) {
        // Pour la M16A4, le bonus headshot PaP est le même que le bonus de dégâts normal PaP
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§dL'Idiot Suicidaire", "§dThe Suicidal Idiot")
            : getTranslatedMessage("§2M16A4", "§2M16A4"); // M16A4 is likely a proper noun, so no translation needed
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        // Translate the tooltip dynamically
        tips.add(Component.literal(getTranslatedMessage("§eFusil d'assaut semi-auto", "§eSemi-auto assault rifle")));
        if (upgraded) {
