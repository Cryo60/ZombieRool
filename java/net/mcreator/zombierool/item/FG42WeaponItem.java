...
public class FG42WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {
    ...
    private static final int COOLDOWN_TICKS = 2; // Cadence de tir modérée (5 tirs/sec)
    private static final int RELOAD_TIME = 65; // 3.25 secondes pour recharger
    
    private static final float WEAPON_DAMAGE = 14.0f; // Bons dégâts de base, entre un SMG et un LMG
    private static final float PAP_BONUS_DAMAGE = 15.0f; // Bon bonus PaP
    
    private static final float BASE_HEADSHOT_DAMAGE = 2.8f; // Très bon multiplicateur headshot
    private static final float PAP_HEADSHOT_BONUS = 0.7f; // Bon bonus headshot si PaP
    
    // Paramètres de tir (projectile invisible)
    private static final float BASE_PROJECTILE_VELOCITY = 3.5f; // Vitesse des balles élevée pour la portée
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.15f; 
    private static final float BASE_PROJECTILE_SPREAD = 0.7f; // Dispersion faible, bonne pour la précision
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.7f; // Précision améliorée avec PaP

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 1.6f; // Recul vertical initial TRES ÉLEVÉ
    private static final float BASE_RECOIL_YAW = 0.4f; // Recul latéral marqué
    private static final float PAP_RECOIL_MULTIPLIER = 0.5f; // Recul fortement réduit avec PaP
    private static final float RECOIL_DECAY_RATE = 0.95f; // La vitesse à laquelle le recul diminue (peut être géré dans la classe du joueur ou le packet handler)

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.FG42_FIRE.get(); // À définir dans ZombieroolModSounds
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.FG42_RELOADING.get(); // À définir
    ...
    public FG42WeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.RARE)); // Rareté Rare pour une arme unique
    }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§9Tueur d'Élite", "§9Elite Killer") // Translated PaP name
            : getTranslatedMessage("§fFG42", "§fFG42"); // FG42 name likely same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§l§eFusil Mitrailleur Lourd / Fusil de Précision", "§l§eHeavy Machine Gun / Sniper Rifle")));
        tips.add(Component.literal(getTranslatedMessage("§aPrécision de fusil de sniper avec la cadence de tir d'une mitraillette.", "§aSniper rifle accuracy with a submachine gun's fire rate.")));
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : Tueur d'Élite", "§dPaP Name: Elite Killer"))); // Specific PaP name - now translated
            ...
            tips.add(Component.literal(getTranslatedMessage("§dRecul et dispersion drastiquement réduits.", "§dDrastically reduced recoil and spread.")));
        }
    }
