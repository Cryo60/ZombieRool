...
public class BARWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO = 20; // Chargeur standard de 20 balles
    private static final int MAX_RESERVE = 160; // 8 chargeurs en réserve
    private static final int COOLDOWN_TICKS = 3; // Cadence de tir plus lente (3.33 tirs/sec)
    private static final int RELOAD_TIME = 60; // 3 secondes pour recharger
    
    private static final float WEAPON_DAMAGE = 18.0f; // Dégâts de base ÉLEVÉS
    private static final float PAP_BONUS_DAMAGE = 20.0f; // Très bon bonus PaP pour les dégâts
    
    private static final float BASE_HEADSHOT_DAMAGE = 3.0f; // Excellent multiplicateur headshot de base
    private static final float PAP_HEADSHOT_BONUS = 0.5f; // Petit bonus headshot si PaP
    
    // Paramètres de tir (projectile invisible)
    private static final float BASE_PROJECTILE_VELOCITY = 4.0f; // Vitesse des balles très rapide
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; 
    private static final float BASE_PROJECTILE_SPREAD = 0.5f; // Très faible dispersion de base
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.8f; // Légèrement meilleure précision avec PaP

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 1.5f; // Recul vertical ÉLEVÉ
    private static final float BASE_RECOIL_YAW = 0.3f; // Recul latéral notable
    private static final float PAP_RECOIL_MULTIPLIER = 0.6f; // Recul fortement réduit avec PaP

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.BAR_FIRE.get(); // À définir dans ZombieroolModSounds
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.BAR_RELOADING.get(); // À définir
    ...
    public BARWeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.UNCOMMON)); // Rareté peu commune pour une arme puissante
    }
    ...
    private static String getTranslatedMessage(String frenchMessage, String englishMessage) {
        // FIX APPLIED HERE: Swapped frenchMessage and englishMessage
    ...
    public Component getName(ItemStack stack) {
        ...
        // The fix is applied within getTranslatedMessage
        String name = upgraded
            ? getTranslatedMessage("§4Bête de Poudre", "§4Powder Beast") // Translated PaP name
            : getTranslatedMessage("§fBAR", "§fBAR"); // BAR name likely same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        ...
        boolean upgraded = isPackAPunched(stack);
        // The fix is applied within getTranslatedMessage
        tips.add(Component.literal(getTranslatedMessage("§l§eFusil Automatique Lourd", "§l§eHeavy Automatic Rifle")));
        tips.add(Component.literal(getTranslatedMessage("§aPeut gérer les hordes de zombies à moyenne et longue portée avec une précision mortelle.", "§aCan handle zombie hordes at medium to long range with deadly accuracy.")));
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : Bête de Poudre", "§dPaP Name: Powder Beast"))); // Specific PaP name - now translated
