...
import net.minecraft.world.item.UseAnim; // Correct import
...
public class M40A3WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    // --- Statistiques de base du M40A3 ---
    private static final int MAX_AMMO = 7; // Moins de balles par chargeur, comme un vrai bolt-action
    private static final int MAX_RESERVE = MAX_AMMO * 6; // Bonne réserve
    private static final int COOLDOWN_TICKS = 20; // Cadence de tir plus lente (1 seconde par tir, bolt-action)
    private static final int RELOAD_TIME = 70; // Temps de rechargement modéré (3.5 secondes)

    private static final float WEAPON_DAMAGE = 26.5f; // Dégâts de base élevés, entre Dragunov et Barret
    private static final float PAP_BONUS_DAMAGE = 25.0f; // Bon bonus de dégâts après Pack-a-Punch

    private static final float BASE_HEADSHOT_DAMAGE = 80.0f; // One-shot les zombies standards très longtemps
    private static final float PAP_HEADSHOT_BONUS = 50.0f; // Énorme bonus headshot PaP pour les vagues avancées

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 4.5f; // Vitesse de la balle rapide
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.12f; // Vitesse améliorée PaP
    private static final float BASE_PROJECTILE_SPREAD = 0.04f; // Très très précis
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.3f; // Ultra précis après PaP (0.012)

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 1.2f; // Recul vertical notable
    private static final float BASE_RECOIL_YAW = 0.15f; // Peu de recul horizontal
    private static final float PAP_RECOIL_MULTIPLIER = 0.35f; // Recul très réduit après PaP

    // Propriétés spécifiques au M40A3 (si besoin)

    // --- Sons de l'arme ---
    // ATTENTION : Assure-toi que ces SoundEvent sont bien définis dans ZombieroolModSounds.java
    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.M40A3_FIRE.get();
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.M40A3_FIRE.get(); // Peut être générique
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.M40A3_RELOADING.get();
    // private static final SoundEvent BOLT_SOUND = ZombieroolModSounds.M40A3_BOLT.get(); // Commenté ou supprimé
    private static final SoundEvent DRY_FIRE_SOUND = ZombieroolModSounds.RIFLE_DRY.get(); // Son de tir à vide
    private static final SoundEvent WEAPON_IN_HAND_SOUND = ZombieroolModSounds.WEAPON_IN_HAND.get(); // Son quand on équipe l'arme

    // --- NBT Tags pour l'état de l'arme ---
    ...
    // --- Classe interne pour Cherry Cola Effects (si utilisée) ---
    ...
    public M40A3WeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.EPIC)); // Rareté Épique pour une arme aussi puissante et emblématique
    }
    ...
    public void tickReload(ItemStack stack, Player player, Level level) {
        // La logique de rechargement est gérée dans inventoryTick
    }


    // --- Gestion des NBT Tags (Ammo, Reserve, ReloadTimer) ---
    ...
    public void startReload(ItemStack stack, Player player) {
        ...
        if (getReloadTimer(stack) == 0 && getAmmo(stack) < MAX_AMMO && getReserve(stack) > 0) {
            ...
            if (!player.level().isClientSide) {
                ...
                // Effet Cherry Cola au rechargement (si activé)
    ...
    // --- Comportement d'utilisation de l'objet (clic droit maintenu) ---

    @Override public UseAnim getUseAnimation(ItemStack s) { return getReloadTimer(s) > 0 ? UseAnim.NONE : UseAnim.BOW; }
    @Override public int getUseDuration(ItemStack s) { return 72000; } // Durée très longue pour permettre le maintien du clic
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        if (getReloadTimer(stack) > 0) { // Si en train de recharger, ne rien faire
            return;
        }

        long lastFireTick = getLastFireTick(stack);
        long currentTick = level.getGameTime();
        if (currentTick - lastFireTick < COOLDOWN_TICKS) { // Vérifier le cooldown
            return;
        }

        if (getAmmo(stack) == 0 && getReserve(stack) > 0) { // Si plus de balles mais de la réserve, recharger
            startReload(stack, player);
            return;
        }

        if (getAmmo(stack) == 0 && getReserve(stack) == 0) { // Plus de balles du tout
        ...
        if (getAmmo(stack) > 0) { // Si balles disponibles, tirer
            ...
            Arrow arrow = new Arrow(level, player); // Utilise une flèche vanilla comme base
            arrow.setOwner(player);
            arrow.getPersistentData().putUUID("shooterUUID", player.getUUID()); // Utile pour identifier l'origine
            arrow.setPos(start.x, start.y, start.z);
            arrow.setSilent(true); // Pas de bruit par défaut de la flèche
            arrow.setNoGravity(true); // Pas d'effet de gravité
            arrow.pickup = AbstractArrow.Pickup.DISALLOWED; // Ne pas pouvoir ramasser la flèche
            arrow.getPersistentData().putBoolean("zombierool:invisible", true); // Rend le projectile invisible (pour simuler une balle)
            arrow.getPersistentData().putBoolean("zombierool:small", true); // Peut-être un tag pour réduire sa taille visuelle
            arrow.setInvisible(true); // Assure l'invisibilité
            arrow.setBaseDamage(0.0D); // Les dégâts sont gérés par ArrowImpactHandler

            arrow.shoot(dir.x, dir.y, dir.z, projectileVelocity, projectileSpread);

            level.addFreshEntity(arrow); // Ajoute la flèche au monde

            // Envoi du paquet de recul au client
            if (player instanceof ServerPlayer serverPlayer) {
                float actualPitchRecoil = recoilPitch;
                float actualYawRecoil = (player.getRandom().nextBoolean() ? 1 : -1) * recoilYaw; // Recul horizontal aléatoire
            ...
            // Ancienne ligne pour le son de l'action du verrou (maintenant retirée)
            // if (BOLT_SOUND != null) { level.playSound(null, player.getX(), player.getY(), player.getZ(), BOLT_SOUND, SoundSource.PLAYERS, 0.7f, 1f); }

            setAmmo(stack, getAmmo(stack) - 1); // Décrémente les munitions
            setLastFireTick(stack, currentTick); // Met à jour le dernier tick de tir
        }
    }

    // --- Logique du tick d'inventaire (gestion rechargement et son d'équipement) ---
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity ent, int slot, boolean sel) {
        ...
        if (level.isClientSide) {
            ...
            if (t == expectedReloadTime && p.hasEffect(ZombieroolModMobEffects.PERKS_EFFECT_CHERRY.get())) {
                // Si tu as une animation spécifique pour Cherry Cola, active-la ici.
                // CherryReloadAnimationHandler.startCherryAnimation(null); // Décommenter si utilisé
            }
            return;
        }

        if (t > 0) { // Si le rechargement est en cours
            // Si l'arme n'est plus sélectionnée ou n'est plus dans la main principale, annuler le rechargement
            if (!sel || !p.getItemInHand(InteractionHand.MAIN_HAND).equals(stack)) {
                setReloadTimer(stack, 0);
                return;
            }
            setReloadTimer(stack, --t); // Décrémente le timer de rechargement
            if (t <= 0) { // Si le rechargement est terminé
        ...
        // Gérer le son d'équipement de l'arme
        if (sel && !wasEquippedPreviously) {
            if (p.getItemInHand(InteractionHand.MAIN_HAND).equals(stack)) { // Assure que c'est bien l'arme équipée
    ...
    // --- Apparence et texte d'info de l'objet ---
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§5Le Chasseur Silencieux", "§5The Silent Hunter") // Translated PaP name
            : getTranslatedMessage("§7M40A3", "§7M40A3"); // Base name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§7Fusil de précision à verrou américain", "§7American Bolt-Action Sniper Rifle")));
        tips.add(Component.literal(getTranslatedMessage("§fLa précision et la puissance brute.", "§fRaw precision and power.")));
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§5Améliorée via Pack-a-Punch", "§5Upgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage(
                "§5Bonus dégâts PaP : " + String.format("%.1f", getHeadshotPapBonusDamage(stack)),
                "§5PaP Damage Bonus: " + String.format("%.1f", getHeadshotPapBonusDamage(stack))
            )));
            tips.add(Component.literal(getTranslatedMessage("§fUn coup, un mort.", "§fOne shot, one kill.")));
        } else {
            tips.add(Component.literal(getTranslatedMessage("§fUn sniper fiable et puissant, parfait pour le headshot.", "§fA reliable and powerful sniper, perfect for headshots.")));
        }
    }
