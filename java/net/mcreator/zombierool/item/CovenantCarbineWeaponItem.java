...
import net.minecraft.world.entity.Entity; // Corrected import for Entity
...
public class CovenantCarbineWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    // --- Statistiques de l'arme (5/10) ---
    private static final int MAX_AMMO       = 18; // Chargeur décent pour une carabine
    private static final int MAX_RESERVE    = MAX_AMMO * 6; // Bonnes réserves
    private static final int COOLDOWN_TICKS = 5; // Cadence de tir moyenne (légèrement plus lent qu'un M1911)
    private static final int RELOAD_TIME    = 40; // Temps de rechargement moyen

    private static final float WEAPON_DAMAGE = 9.5f; // Dégâts solides mais pas surpuissants (entre M1911 et Magnum)
    private static final float PAP_BONUS_DAMAGE = 3.5f; // Bon bonus pour le Pack-a-Punch

    private static final float BASE_HEADSHOT_DAMAGE = 1.8f; // Bon multiplicateur de headshot
    private static final float PAP_HEADSHOT_BONUS = 2.8f;    // Excellent bonus si PaP

    // Bullet Parameters
    private static final float BASE_PROJECTILE_VELOCITY = 1.9f; // Vitesse de projectile standard
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Vitesse légèrement augmentée
    private static final float BASE_PROJECTILE_SPREAD = 1.2f; // Dispersion contrôlable
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.7f; // Précision améliorée avec PaP

    // Recoil Parameters
    private static final float BASE_RECOIL_PITCH = 0.9f; // Recul vertical modéré
    private static final float BASE_RECOIL_YAW = 0.5f;   // Recul horizontal faible
    private static final float PAP_RECOIL_MULTIPLIER = 0.8f; // Recul réduit avec PaP

    // --- Sons de l'arme ---
    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.CARBINE_FIRE.get(); // À remplacer par ton son spécifique
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // À remplacer
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.CARBINE_RELOADING.get(); // À remplacer
    private static final SoundEvent DRY_FIRE_SOUND       = ZombieroolModSounds.RIFLE_DRY.get();
    private static final SoundEvent WEAPON_IN_HAND_SOUND = ZombieroolModSounds.WEAPON_IN_HAND.get(); // À remplacer
    ...
    public CovenantCarbineWeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.COMMON)); // Toujours 1 stackable pour les armes
    }
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        if (getAmmo(stack) > 0) {
            ...
            arrow.getPersistentData().putBoolean("zombierool:small", true); // Peut-être plus adapté à une "balle"
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§dRéfutateur", "§dRefuter") // Translated PaP name
            : getTranslatedMessage("§aCarabine Covenant", "§aCovenant Carbine"); // Base name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eArme Covenant précise, excellente pour les tirs à la tête.", "§ePrecise Covenant weapon, excellent for headshots."))); // Description
        if (upgraded) {
            ...
            tips.add(Component.literal(getTranslatedMessage(
                "§dBonus dégâts PaP : " + String.format("%.1f", PAP_BONUS_DAMAGE), // Changed from getHeadshotPapBonusDamage to PAP_BONUS_DAMAGE for clarity
