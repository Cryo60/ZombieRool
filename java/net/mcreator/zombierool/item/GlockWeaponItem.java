...
public class GlockWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon, IHandgunWeapon {

    private static final int MAX_AMMO = 17; // Standard Glock magazine capacity
    private static final int MAX_RESERVE = MAX_AMMO * 6; // Decent reserve for a pistol
    private static final int COOLDOWN_TICKS = 6; // Quick firing rate (0.3 seconds per shot)
    private static final int RELOAD_TIME = 30; // 1.5 seconds for a full reload
    
    private static final float WEAPON_DAMAGE = 5.0f; // More damage than M1911 (4.0) but less than Mauser (6.0)
    private static final float PAP_BONUS_DAMAGE = 7.0f; // Good bonus for PaP, bringing it to 12.0f
    
    private static final float BASE_HEADSHOT_DAMAGE = 2.5f; // Standard pistol headshot multiplier
    private static final float PAP_HEADSHOT_BONUS = 1.0f;    // No extra headshot multiplier on PaP (relying on base damage)

    // Firing parameters
    private static final float BASE_PROJECTILE_VELOCITY = 3.5f; // Faster than M1911
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; 
    private static final float BASE_PROJECTILE_SPREAD = 0.5f; // Moderate spread, better than M1911, worse than Mauser/Kar98k
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.7f; // Improves accuracy when PaP'd

    // Recoil parameters
    private static final float BASE_RECOIL_PITCH = 0.8f; // Manageable recoil
    private static final float BASE_RECOIL_YAW = 0.4f;   
    private static final float PAP_RECOIL_MULTIPLIER = 0.5f; // PaP significantly reduces recoil

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.GLOCK_FIRE.get(); // Ensure these sounds exist
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.GLOCK_RELOADING.get();
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        // If the weapon is currently reloading, prevent firing
        if (getReloadTimer(stack) > 0) {
            return;
        }

        // If the cooldown is not finished, prevent firing
        if (currentTick - lastFireTick < COOLDOWN_TICKS) {
            return;
        }

        // If no ammo in magazine but reserve exists, start a full reload
        if (getAmmo(stack) == 0 && getReserve(stack) > 0) {
            startReload(stack, player);
            return;
        }
        
        // If completely out of ammo (dry fire)
        if (getAmmo(stack) == 0 && getReserve(stack) == 0) {
        ...
        if (getAmmo(stack) > 0) {
            ...
            arrow.setBaseDamage(0.0D); // Gérer par ArrowImpactHandler
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity ent, int slot, boolean sel) {
        ...
        // Server-side logic for reload timer
        if (t > 0) {
            // If weapon is unselected, interrupt reload
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§eG18 : Full Auto", "§eG18: Full Auto") // PaP name suggestion
            : getTranslatedMessage("§7Glock", "§7Glock"); // Glock name likely same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§ePistolet fiable. Bon pour les premières vagues.", "§eReliable pistol. Good for early waves.")));
        if (upgraded) {
