...
public class AK74uWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 30; // Chargeur standard SMG
    private static final int MAX_RESERVE    = MAX_AMMO * 9; // Très bonne réserve pour un SMG rapide
    private static final int COOLDOWN_TICKS = 2; // Très rapide, comme la M16A4, 2 ticks = 0.1 secondes
    private static final int RELOAD_TIME    = 50; // Rechargement rapide (2.5s)

    private static final float WEAPON_DAMAGE = 7.5f; // Dégâts légèrement inférieurs à l'AK-47
    private static final float PAP_BONUS_DAMAGE = 9.0f; // Bon bonus PaP pour compenser

    private static final float BASE_HEADSHOT_DAMAGE = 11.0f; // Tue en un coup de headshot jusqu'à 11 cœurs (vague 4)
    private static final float PAP_HEADSHOT_BONUS = 9.0f; // Bonus headshot PaP

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 2.8f; // Vitesse de projectile plus faible (SMG)
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.25f; // Bon gain de vitesse avec PaP
    private static final float BASE_PROJECTILE_SPREAD = 0.7f; // Dispersion moyenne
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.5f; // Amélioration notable de la précision

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 0.8f; // Recul en haut/bas modéré, mais rapide
    private static final float BASE_RECOIL_YAW = 0.5f;   // Recul gauche/droite modéré
    private static final float PAP_RECOIL_MULTIPLIER = 0.4f; // Forte réduction du recul avec PaP

    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.AK74_FIRE.get(); // À créer : son de tir AK-74u
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.AK74_RELOADING.get(); // À créer : son de rechargement AK-74u
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        if (getAmmo(stack) > 0) {
            ...
            level.addFreshEntity(arrow); // Added the missing line
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§eLe Dévoreur Compact", "§eThe Compact Devourer") // Translated PaP name
            : getTranslatedMessage("§6AK-74u", "§6AK-74u"); // AK-74u name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eMitraillette compacte", "§eCompact Submachine Gun")));
        tips.add(Component.literal(getTranslatedMessage("§aCadence de tir élevée et maniabilité pour les combats rapprochés.", "§aHigh rate of fire and maneuverability for close-quarters combat."))); // Added descriptive text
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : Le Dévoreur Compact", "§dPaP Name: The Compact Devourer"))); // Specific PaP name - now translated
