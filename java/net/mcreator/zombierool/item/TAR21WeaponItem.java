...
public class TAR21WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 30;
    private static final int MAX_RESERVE    = MAX_AMMO * 7; // Bonnes réserves
    private static final int COOLDOWN_TICKS = 3; // Cadence de tir rapide (0.15s) pour une 6.5/10
         
    private static final int RELOAD_TIME    = 65; // ticks (3.25s)
         
    private static final float WEAPON_DAMAGE = 7.0f; // Dégâts solides, un peu moins que l'AUG
    private static final float PAP_BONUS_DAMAGE = 9.0f; // Bon bonus PaP
         
    private static final float BASE_HEADSHOT_DAMAGE = 15.0f; // Dégâts de base du headshot respectables
    private static final float PAP_HEADSHOT_BONUS = 10.0f;    // Bon bonus headshot si PaP

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 3.2f; // Vitesse de projectile correcte
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Bon multiplicateur de vitesse pour PaP
    private static final float BASE_PROJECTILE_SPREAD = 0.4f; // Précision modérée
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.65f; // Amélioration notable de la précision avec PaP

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 0.7f; // Recul vertical similaire à la M16A4
    private static final float BASE_RECOIL_YAW = 0.35f;   // Recul horizontal modéré
    private static final float PAP_RECOIL_MULTIPLIER = 0.6f; // Recul bien réduit avec PaP


    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.TAR21_FIRE.get(); // Assure-toi d'avoir ce son
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.TAR21_RELOADING.get(); // Assure-toi d'avoir ce son
    ...
    public TAR21WeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.UNCOMMON)); // Rareté UNCOMMON
    }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§6Fusil à Pompe", "§6Pump-Action Shotgun") // Translated PaP name
            : getTranslatedMessage("§aTAR-21", "§aTAR-21"); // TAR-21 name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        ...
        tips.add(Component.literal(getTranslatedMessage("§aCompact et fiable, idéal pour les combats à moyenne portée.", "§aCompact and reliable, ideal for medium-range combat."))); // Added descriptive text
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : Fusil à Pompe", "§dPaP Name: Pump-Action Shotgun"))); // Specific PaP name - now translated
