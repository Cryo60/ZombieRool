...
public class FAMASWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 30; // Chargeur standard, comme les autres AR
    private static final int MAX_RESERVE    = MAX_AMMO * 7; // Réserve légèrement moins grande que l'AK-47, mais correcte
    private static final int COOLDOWN_TICKS = 1; // EXTRÊMEMENT rapide pour un AR, 1 tick = 0.05 secondes
    private static final int RELOAD_TIME    = 60; // Rechargement moyen (3.0s), comme la M16A4

    private static final float WEAPON_DAMAGE = 6.0f; // Dégâts solides par balle, équivalents à la M16A4
    private static final float PAP_BONUS_DAMAGE = 12.0f; // Très bon bonus PaP pour la faire briller

    private static final float BASE_HEADSHOT_DAMAGE = 18.0f; // Très bon headshot de base, one-shot les zombies jusqu'à 18 cœurs (vague 7)
    private static final float PAP_HEADSHOT_BONUS = 12.0f; // Très bon bonus headshot PaP

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 3.5f; // Très haute vitesse de projectile
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Bon gain de vitesse
    private static final float BASE_PROJECTILE_SPREAD = 0.4f; // Très bonne précision de base (pour un AR)
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.4f; // Amélioration MASSIVE de la précision avec PaP

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 0.6f; // Recul en haut/bas modéré, mais à gérer avec la cadence
    private static final float BASE_RECOIL_YAW = 0.4f;   // Recul gauche/droite faible
    private static final float PAP_RECOIL_MULTIPLIER = 0.3f; // Réduction TRÈS forte du recul avec PaP

    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.FAMAS_FIRE.get(); // À créer : son de tir FAMAS
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.FAMAS_RELOADING.get(); // À créer : son de rechargement FAMAS
            ...
            if (!player.level().isClientSide) {
                ...
                if (player.hasEffect(ZombieroolModMobEffects.PERKS_EFFECT_CHERRY.get())) {
                    ...
                    for (LivingEntity target : entities) {
                        // Corrected to include Animal in the check for non-tamed animals
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§9G11000", "§9G11000") // Proper noun, remains same
            : getTranslatedMessage("§aFAMAS", "§aFAMAS"); // Proper noun, remains same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eFusil d'assaut à haute cadence de tir", "§eHigh Rate of Fire Assault Rifle")));
        tips.add(Component.literal(getTranslatedMessage("§aPrécision et puissance, idéale pour les situations de combat intenses.", "§aAccuracy and power, ideal for intense combat situations."))); // Added descriptive text
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : G11000", "§dPaP Name: G11000"))); // Specific PaP name - remains same
