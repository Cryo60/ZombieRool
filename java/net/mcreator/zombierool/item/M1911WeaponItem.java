...
public class M1911WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon, IHandgunWeapon { // Re-added IHandgunWeapon

    private static final int MAX_AMMO       = 8; // M1911 has 8 rounds in the magazine
    private static final int MAX_RESERVE    = MAX_AMMO * 8; // Adjust reserve ammo accordingly
    private static final int COOLDOWN_TICKS = 5; // Slower fire rate for a pistol
    private static final int RELOAD_TIME    = 30; // ticks (2s for a pistol reload)
    
    private static final float WEAPON_DAMAGE = 0.4f; // Very low damage as requested
    private static final float PAP_BONUS_DAMAGE = 3.0f; // Example Pack-a-Punch damage bonus
    
    private static final float BASE_HEADSHOT_DAMAGE = 1.0f; // Base headshot damage (multiplies base)
    private static final float PAP_HEADSHOT_BONUS = 2.0f;    // Bonus headshot damage if PaP

    // Bullet Parameters
    private static final float BASE_PROJECTILE_VELOCITY = 2.0f; // M1911 projectile speed
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Speed multiplier for PaP
    private static final float BASE_PROJECTILE_SPREAD = 1.0f; // Base spread for a pistol
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.7f; // Spread multiplier for PaP (more accurate)

    // Recoil Parameters
    private static final float BASE_RECOIL_PITCH = 0.9f; // Base recoil up/down (pitch)
    private static final float BASE_RECOIL_YAW = 0.5f;   // Base recoil left/right (yaw)
    private static final float PAP_RECOIL_MULTIPLIER = 0.8f; // Recoil multiplier for PaP (less recoil)


    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.M1911_FIRE.get(); // M1911 fire sound
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Example upgraded sound
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.M1911_RELOADING.get(); // M1911 reload sound
    private static final SoundEvent DRY_FIRE_SOUND       = ZombieroolModSounds.RIFLE_DRY.get(); // Using generic rifle dry fire for now
    private static final SoundEvent WEAPON_IN_HAND_SOUND = ZombieroolModSounds.WEAPON_IN_HAND.get(); // Generic weapon in hand sound
    // private static final SoundEvent RELOAD_FINISH_SOUND  = ZombieroolModSounds.DOOR_OPENING_OLD.get(); // Removed reload finish sound as requested
            ...
            if (!player.level().isClientSide) { // This block runs on the server
                ...
                if (player.hasEffect(ZombieroolModMobEffects.PERKS_EFFECT_CHERRY.get())) {
                    ...
                    for (LivingEntity target : entities) {
                        // Ensure the target is a monster or an untamed animal, or not an animal at all
    ...
    private void finishReload(ItemStack stack, Player player, Level level) {
        ...
        // Removed reload finish sound as requested
        sp.connection.send(new ClientboundSetEquipmentPacket(
            sp.getId(), List.of(Pair.of(EquipmentSlot.MAINHAND, stack))));
    }
    ...
    @Override public UseAnim getUseAnimation(ItemStack s) { return getReloadTimer(s) > 0 ? UseAnim.NONE : UseAnim.BOW; } // Still using BOW animation
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        if (getAmmo(stack) > 0) {
            ...
            Arrow arrow = new Arrow(level, player); // Using Arrow for projectile, can be replaced with custom bullet entity
            arrow.setOwner(player);
            arrow.getPersistentData().putUUID("shooterUUID", player.getUUID());
            arrow.setPos(start.x, start.y, start.z);
            arrow.setSilent(true);
            arrow.setNoGravity(true); // Bullets typically don't have gravity
            arrow.pickup = Pickup.DISALLOWED; // Bullets are not picked up
            arrow.getPersistentData().putBoolean("zombierool:invisible", true); // Keep invisible for your custom bullet rendering
            arrow.getPersistentData().putBoolean("zombierool:small", true);     // Keep small for your custom bullet rendering
            arrow.setInvisible(true); // Ensure it's invisible
            arrow.setBaseDamage(0.0D); // Base damage is handled by your impact handler, not the arrow itself

            if (isPackAPunched(stack)) {
                // Add any specific NBT tags for your upgraded bullet,
                // for instance, if it causes explosions or fire
                arrow.setSecondsOnFire(200); // Example: if PaP bullets ignite targets
                arrow.getPersistentData().putBoolean("ignoreFireResistance", true);
                arrow.getPersistentData().putBoolean("ignoreRain", true);
                arrow.getPersistentData().putBoolean("zombierool:m1911_pap", true); // Add specific tag for your handler
            }
    ...
    public Component getName(ItemStack stack) {
        ...
        // Changed name for M1911 with dynamic translation
        String name = upgraded
            ? getTranslatedMessage("§dLe Mustang et Sally", "§dThe Mustang & Sally")
            : getTranslatedMessage("§2M1911", "§2M1911"); // M1911 name is likely the same in both languages for a specific weapon
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§ePistolet semi-automatique", "§eSemi-automatic pistol"))); // Pistol description
        if (upgraded) {
            ...
            // Add specific tooltip for M1911 PaP if needed, e.g., explosive rounds
            tips.add(Component.literal(getTranslatedMessage("§4Projectiles explosifs", "§4Explosive Projectiles")));
        }
    }
