...
import net.mcreator.zombierool.init.ZombieroolModSounds; // Assurez-vous que vos sons sont bien enregistrés ici
...
// Ensure this class extends IHandgunWeapon if the SMG is considered a handgun for specific perks/mechanics,
// or create an ISMGWeapon if it has its own unique interfaces.
// For now, I'll remove IHandgunWeapon as SMG is distinct from pistol.
public class M7SMGWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon { // Removed IHandgunWeapon

    private static final int MAX_AMMO       = 60; // SMG has a large magazine
    private static final int MAX_RESERVE    = MAX_AMMO * 4; // Decent reserve ammo
    private static final int COOLDOWN_TICKS = 2; // Fast fire rate for an SMG (2 ticks = 10 shots/second)
    private static final int RELOAD_TIME    = 40; // ticks (2s for a pistol reload, 2.5s for SMG)
      
    private static final float WEAPON_DAMAGE = 1.0f; // Moderate damage for an SMG
    private static final float PAP_BONUS_DAMAGE = 4.0f; // Good Pack-a-Punch damage bonus
      
    private static final float BASE_HEADSHOT_DAMAGE = 1.5f; // Base headshot damage (multiplies base)
    private static final float PAP_HEADSHOT_BONUS = 2.5f;    // Bonus headshot damage if PaP

    // Bullet Parameters
    private static final float BASE_PROJECTILE_VELOCITY = 2.5f; // SMG projectile speed
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Speed multiplier for PaP
    private static final float BASE_PROJECTILE_SPREAD = 1.5f; // Base spread for an SMG (moderate)
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.8f; // Spread multiplier for PaP (more accurate)

    // Recoil Parameters
    private static final float BASE_RECOIL_PITCH = 0.7f; // Base recoil up/down (pitch)
    private static final float BASE_RECOIL_YAW = 0.8f;   // Base recoil left/right (yaw)
    private static final float PAP_RECOIL_MULTIPLIER = 0.7f; // Recoil multiplier for PaP (less recoil)


    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.SMG_LOOP.get(); // SMG fire sound (continuous loop)
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Example upgraded sound
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.SMG_RELOADING.get(); // SMG reload sound
    private static final SoundEvent DRY_FIRE_SOUND       = ZombieroolModSounds.RIFLE_DRY.get(); // Using generic rifle dry fire for now
    private static final SoundEvent WEAPON_IN_HAND_SOUND = ZombieroolModSounds.SMG_READY.get(); // SMG ready sound
    ...
    // Cherry Cola Effects (consistent across weapons)
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity ent, int slot, boolean sel) {
        ...
        // Logic for playing the "weapon in hand" sound (smg_ready)
    ...
    public Component getName(ItemStack stack) {
        ...
        // Changed name for M7 SMG
        String name = upgraded
            ? getTranslatedMessage("§dBlaze of Glory", "§dBlaze of Glory") // PaP name, remains same as it's a proper noun-like name
            : getTranslatedMessage("§2M7 SMG", "§2M7 SMG"); // Base name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eMitraillette légère et rapide de la UNSC", "§eLight and fast UNSC Submachine Gun"))); // SMG description
        if (upgraded) {
