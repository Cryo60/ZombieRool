...
public class FNFALWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 20; // Smaller magazine for a harder-hitting rifle
    private static final int MAX_RESERVE    = MAX_AMMO * 6; // Standard reserves
    private static final int COOLDOWN_TICKS = 8; // Slower semi-automatic fire rate (0.4s)
         
    private static final int RELOAD_TIME    = 55; // ticks (2.75s) - relatively quick reload
         
    private static final float WEAPON_DAMAGE = 8.0f; // High base damage per shot for a 4/10
    private static final float PAP_BONUS_DAMAGE = 7.0f; // Decent but not outstanding PaP bonus
         
    private static final float BASE_HEADSHOT_DAMAGE = 18.0f; // Good headshot multiplier to reward precision
    private static final float PAP_HEADSHOT_BONUS = 8.0f;    // Solid headshot bonus with PaP

    // Bullet Parameters
    private static final float BASE_PROJECTILE_VELOCITY = 3.8f; // Fast projectile velocity
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Slight increase with PaP
    private static final float BASE_PROJECTILE_SPREAD = 0.2f; // Very precise base spread for semi-auto
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.7f; // Even tighter with PaP

    // Recoil Parameters
    private static final float BASE_RECOIL_PITCH = 1.2f; // Noticeable vertical recoil
    private static final float BASE_RECOIL_YAW = 0.6f;   // Moderate horizontal recoil
    private static final float PAP_RECOIL_MULTIPLIER = 0.7f; // Recoil reduced with PaP


    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.FNFAL_FIRE.get(); // Specific sound
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.FNFAL_RELOADING.get(); // Specific sound
    ...
    // --- Inner class for Cherry Cola Effects ---
    ...
    public FNFALWeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.COMMON)); // Common rarity
    }
    ...
    // IHeadshotWeapon implementation
    @Override
    ...
    public Component getName(ItemStack stack) {
        ...
        // Nom PaP stylisé pour la FN FAL : "Le Vengeur Belge" ou "Force de Frappe"
        String name = upgraded
            ? getTranslatedMessage("§cLe Vengeur Belge", "§cThe Belgian Avenger") // Translated PaP name
            : getTranslatedMessage("§aFN FAL", "§aFN FAL"); // FN FAL name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eFusil d'assaut semi-automatique", "§eSemi-Automatic Assault Rifle")));
        tips.add(Component.literal(getTranslatedMessage("§aPrécision et puissance pour les tireurs d'élite.", "§aAccuracy and power for marksmen."))); // Added descriptive text
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : Le Vengeur Belge", "§dPaP Name: The Belgian Avenger"))); // Specific PaP name - now translated
