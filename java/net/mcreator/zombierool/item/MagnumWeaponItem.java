...
public class MagnumWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon { // Removed IHandgunWeapon as it was not defined in the provided code

    private static final int MAX_AMMO       = 8; // Le Magnum a 8 balles
    private static final int MAX_RESERVE    = MAX_AMMO * 10; // Un peu plus de réserves que le M1911
    private static final int COOLDOWN_TICKS = 7; // Tir un peu plus lent que le M1911 mais plus impactant
    private static final int RELOAD_TIME    = 35; // Ticks (légèrement plus long que le M1911)
    
    private static final float WEAPON_DAMAGE = 0.6f; // Dégâts légèrement supérieurs au M1911 (0.4f)
    private static final float PAP_BONUS_DAMAGE = 4.0f; // Bon bonus de dégâts pour le PaP
    
    private static final float BASE_HEADSHOT_DAMAGE = 1.5f; // Bonus de dégâts de tête plus élevé que le M1911
    private static final float PAP_HEADSHOT_BONUS = 2.5f;    // Gros bonus de dégâts de tête si PaP

    // Bullet Parameters
    private static final float BASE_PROJECTILE_VELOCITY = 2.2f; // Projectile un peu plus rapide
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.15f; // Vitesse augmentée
    private static final float BASE_PROJECTILE_SPREAD = 0.8f; // Moins de dispersion de base
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.6f; // Encore plus précis avec PaP

    // Recoil Parameters
    private static final float BASE_RECOIL_PITCH = 1.2f; // Recul vertical plus prononcé
    private static final float BASE_RECOIL_YAW = 0.7f;   // Recul horizontal notable
    private static final float PAP_RECOIL_MULTIPLIER = 0.7f; // Recul significativement réduit avec PaP


    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.MAGNUM_FIRE.get();
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Peut-être un son spécifique pour le Magnum amélioré si tu en as un
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.MAGNUM_RELOADING.get();
    private static final SoundEvent DRY_FIRE_SOUND       = ZombieroolModSounds.RIFLE_DRY.get(); // Utilise toujours celui-ci si pas de son spécifique
    private static final SoundEvent WEAPON_IN_HAND_SOUND = ZombieroolModSounds.MAGNUM_READY.get(); // Son "ready" quand on le prend en main
    ...
    public MagnumWeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.COMMON)); // Rarity à COMMON comme tu l'as mis
    }
    ...
    private void finishReload(ItemStack stack, Player player, Level level) {
        ...
        // Play the "magnum_ready" sound after reload finishes
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§dMisriah", "§dMisriah") // PaP name, remains same as it's a proper noun-like name
            : getTranslatedMessage("§2M6H2", "§2M6H2"); // Base name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§ePistolet de poing puissant de la UNSC", "§ePowerful UNSC Handgun"))); // Description
        if (upgraded) {
