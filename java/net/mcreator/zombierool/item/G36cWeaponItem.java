...
public class G36cWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 35;   // Chargeur augmenté (Ancien : 30)
    private static final int MAX_RESERVE    = MAX_AMMO * 7; // Plus de chargeurs en réserve (Ancien : 6)
    private static final int COOLDOWN_TICKS = 2; // Cadence de tir très rapide, comme la MP40 (Ancien : 3)
    private static final int RELOAD_TIME    = 40; // Temps de rechargement réduit (Ancien : 50)

    private static final float WEAPON_DAMAGE = 11.5f; // Dégâts de base significativement augmentés (Ancien : 5.0f)
    private static final float PAP_BONUS_DAMAGE = 12.0f;  // Bonus PaP massifs (Ancien : 7.0f)

    private static final float BASE_HEADSHOT_DAMAGE = 15.0f; // Dégâts de headshot de base très élevés (Ancien : 11.0f)
    private static final float PAP_HEADSHOT_BONUS = 9.0f; // Bonus headshot PaP encore plus puissant (Ancien : 7.0f)

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 3.8f; // Vitesse des balles augmentée (Ancien : 3.2f)
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.15f; // Multiplicateur de vitesse PaP amélioré (Ancien : 1.1f)
    private static final float BASE_PROJECTILE_SPREAD = 0.5f; // Dispersion de base grandement réduite (Ancien : 0.7f)
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.3f; // Dispersion quasi inexistante avec PaP (Ancien : 0.6f)

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 0.4f; // Recul vertical très réduit (Ancien : 0.6f)
    private static final float BASE_RECOIL_YAW = 0.2f; // Recul latéral très réduit (Ancien : 0.4f)
    private static final float PAP_RECOIL_MULTIPLIER = 0.5f; // Recul presque nul avec PaP (Ancien : 0.7f)

     private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.G36C_FIRE.get(); // Reusing sounds for now
     private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
     private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.G36C_RELOADING.get();
    ...
    private static String getTranslatedMessage(String frenchMessage, String englishMessage) {
     ...
     public Component getName(ItemStack stack) {
         ...
         String name = upgraded
            ? getTranslatedMessage("§dLe Nettoyeur", "§dThe Cleaner") // Translated PaP name
            : getTranslatedMessage("§2G36C", "§2G36C"); // G36C name (proper noun, remains same)
     ...
     public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
         initializeIfNeeded(stack);
         boolean upgraded = isPackAPunched(stack);
         tips.add(Component.literal(getTranslatedMessage("§eFusil d'assaut compact", "§eCompact Assault Rifle")));
         tips.add(Component.literal(getTranslatedMessage("§aUn fusil d'assaut polyvalent avec une cadence de tir rapide et un bon contrôle.", "§aA versatile assault rifle with a high rate of fire and good control."))); // Added descriptive text
         if (upgraded) {
             tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
             tips.add(Component.literal(getTranslatedMessage("§dNom PaP : Le Nettoyeur", "§dPaP Name: The Cleaner"))); // Specific PaP name - now translated
