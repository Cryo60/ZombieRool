...
public class ArisakaWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO = 5;
    private static final int MAX_RESERVE = MAX_AMMO * 7; // Entre Mosin et Kar98k pour la réserve
    private static final int BOLT_CYCLE_TIME = 40; // Plus rapide que le Mosin, légèrement plus lent que le Kar98k
    private static final int RELOAD_TIME = 30; // Plus rapide que le Mosin, plus lent que le Kar98k
    
    private static final float WEAPON_DAMAGE = 5.5f; // Un peu plus que le Mosin, moins que le Kar98k
    private static final float PAP_BONUS_DAMAGE = 3.0f; 
    
    private static final float BASE_HEADSHOT_DAMAGE = 5.7f; // Multiplicateur légèrement meilleur que le Mosin
    private static final float PAP_HEADSHOT_BONUS = 1.9f; 

    private static final float BASE_PROJECTILE_VELOCITY = 4.8f; // Vitesse de projectile légèrement améliorée
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f;
    private static final float BASE_PROJECTILE_SPREAD = 0.015f; // Légèrement plus précis que le Mosin
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.55f; 

    private static final float BASE_RECOIL_PITCH = 2.6f; // Recul un peu moins prononcé que le Mosin
    private static final float BASE_RECOIL_YAW = 1.1f;
    private static final float PAP_RECOIL_MULTIPLIER = 0.45f; 

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.ARISAKA_FIRE.get(); // À définir dans ZombieroolModSounds
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.ARISAKA_RELOADING.get(); // À définir
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§eLe Souffle du Dragon", "§eThe Dragon's Breath") // Translated PaP name
            : getTranslatedMessage("§7Arisaka", "§7Arisaka"); // Arisaka name likely same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eFusil de précision japonais, fiable et précis.", "§eReliable and precise Japanese sniper rifle.")));
        if (upgraded) {
