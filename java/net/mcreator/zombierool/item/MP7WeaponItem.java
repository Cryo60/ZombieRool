...
public class MP7WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon { // Removed IHandgunWeapon as it was not defined in the provided code

    private static final int MAX_AMMO = 45; // Capacité du chargeur légèrement augmentée (Ancien : 40)
    private static final int MAX_RESERVE = MAX_AMMO * 8; // Réserve toujours très grande (Adaptée au nouveau MAX_AMMO)
    private static final int COOLDOWN_TICKS = 1; // ULTRA HAUTE CADENCE DE TIR (Inchangé)
    private static final int RELOAD_TIME = 45; // Rechargement légèrement plus rapide (Ancien : 50)

    private static final float WEAPON_DAMAGE = 7.0f; // Dégâts de base augmentés (Ancien : 6.0f)
    private static final float PAP_BONUS_DAMAGE = 12.0f; // Bonus Pack-a-Punch augmenté (Ancien : 10.0f)

    private static final float BASE_HEADSHOT_DAMAGE = 3.0f; // Multiplicateur headshot augmenté (Ancien : 2.5f)
    private static final float PAP_HEADSHOT_BONUS = 12.0f; // Dégâts bonus headshot PaP augmentés (Ancien : 10.0f)

    private static final float BASE_PROJECTILE_VELOCITY = 3.7f; // Vitesse de projectile légèrement améliorée (Ancien : 3.5f)
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.3f; // Très bon boost PaP amélioré (Ancien : 1.25f)
    private static final float BASE_PROJECTILE_SPREAD = 0.5f; // Excellente précision de base encore améliorée (Ancien : 0.6f)
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.35f; // Précision quasi parfaite PaP encore plus serrée (Ancien : 0.4f)

    private static final float BASE_RECOIL_PITCH = 0.25f; // Recul vertical ultra faible encore réduit (Ancien : 0.3f)
    private static final float BASE_RECOIL_YAW = 0.15f; // Recul horizontal quasi inexistant encore réduit (Ancien : 0.2f)
    private static final float PAP_RECOIL_MULTIPLIER = 0.15f; // Recul presque nul PaP encore plus inexistant (Ancien : 0.2f)

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.MP7_FIRE.get(); // À définir
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.MP7_FIRE.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.MP7_RELOADING.get(); // À définir
    ...
    public MP7WeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.EPIC)); // Rareté Épique pour une arme 10/10
    }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§cThe Iron Curtain", "§cThe Iron Curtain") // PaP name, remains same as it's a proper noun-like name
            : getTranslatedMessage("§aMP7", "§aMP7"); // Base name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§ePistolet-mitrailleur de haute précision et cadence de tir extrême.", "§eHigh-precision submachine gun with extreme fire rate.")));
        if (upgraded) {
            ...
            tips.add(Component.literal(getTranslatedMessage(
                "§dMultiplicateur Headshot PaP : x" + String.format("%.1f", BASE_HEADSHOT_DAMAGE),
