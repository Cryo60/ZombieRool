...
public class ACRWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 30;
    private static final int MAX_RESERVE    = MAX_AMMO * 9; // Très grandes réserves pour l'endurance
    private static final int COOLDOWN_TICKS = 2; // Cadence de tir EXTRÊMEMENT rapide (0.1s), son point fort
         
    private static final int RELOAD_TIME    = 60; // ticks (3.0s) - rapide pour sa catégorie
         
    private static final float WEAPON_DAMAGE = 8.5f; // Dégâts de base solides, combinés à la cadence de tir
    private static final float PAP_BONUS_DAMAGE = 10.0f; // Très bon bonus PaP, mais ce sont surtout la cadence et la précision qui brillent
         
    private static final float BASE_HEADSHOT_DAMAGE = 18.0f; // Très bons dégâts de base du headshot
    private static final float PAP_HEADSHOT_BONUS = 12.0f;    // Excellent bonus headshot si PaP

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 4.0f; // Vitesse de projectile TRÈS rapide
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.2f; // Excellent multiplicateur de vitesse pour PaP
    private static final float BASE_PROJECTILE_SPREAD = 0.2f; // Précision EXCEPTIONNELLE de base
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.5f; // Précision quasi parfaite avec PaP

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 0.6f; // Recul vertical TRÈS faible (facile à contrôler)
    private static final float BASE_RECOIL_YAW = 0.3f;   // Recul horizontal MINIME
    private static final float PAP_RECOIL_MULTIPLIER = 0.4f; // Recul presqu'inexistant avec PaP


    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.ACR_FIRE.get(); // À créer : son de tir ACR
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.ACR_RELOADING.get(); // À créer : son de rechargement ACR
    ...
    public ACRWeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.EPIC)); // Rareté ÉPIQUE pour une arme 8.5/10
    }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§6La Perfection Opérationnelle", "§6The Operational Perfection") // Translated PaP name
            : getTranslatedMessage("§9ACR", "§9ACR"); // ACR name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eFusil d'assaut automatique ultra-précis", "§eUltra-Precise Automatic Assault Rifle")));
        tips.add(Component.literal(getTranslatedMessage("§aCadence de tir élevée et précision inégalée pour dominer le champ de bataille.", "§aHigh rate of fire and unmatched precision to dominate the battlefield."))); // Added descriptive text
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : La Perfection Opérationnelle", "§dPaP Name: The Operational Perfection"))); // Specific PaP name - now translated
            ...
            tips.add(Component.literal(getTranslatedMessage("§dDispersion et recul presqu'inexistants.", "§dNearly non-existent spread and recoil."))); // Added descriptive text
        }
    }
