...
public class USP45WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon, IHandgunWeapon {

    private static final int MAX_AMMO = 12; // Plus de balles que le M1911
    private static final int MAX_RESERVE = MAX_AMMO * 6; // Réserve correcte pour un pistolet
    private static final int COOLDOWN_TICKS = 4; // Cadence de tir plus rapide que le M1911
    private static final int RELOAD_TIME = 25; // Rechargement plus rapide que le M1911
    
    private static final float WEAPON_DAMAGE = 5.0f; // Dégâts de base significativement plus élevés que le M1911
    private static final float PAP_BONUS_DAMAGE = 5.0f; // Bon bonus Pack-a-Punch
    
    private static final float BASE_HEADSHOT_DAMAGE = 2.0f; // Multiplicateur headshot de base amélioré
    private static final float PAP_HEADSHOT_BONUS = 2.5f; // Bonus headshot si PaP
    
    private static final float BASE_PROJECTILE_VELOCITY = 2.5f; // Vitesse de projectile améliorée
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.15f; 
    private static final float BASE_PROJECTILE_SPREAD = 0.8f; // Moins de dispersion que le M1911
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.6f; 

    private static final float BASE_RECOIL_PITCH = 0.8f; // Moins de recul vertical
    private static final float BASE_RECOIL_YAW = 0.4f; // Moins de recul horizontal
    private static final float PAP_RECOIL_MULTIPLIER = 0.7f; 

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.USP45_FIRE.get(); // À définir dans ZombieroolModSounds
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.USP45_RELOADING.get(); // À définir
    private static final SoundEvent DRY_FIRE_SOUND = ZombieroolModSounds.RIFLE_DRY.get(); // Utilise un son de pistolet vide
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§5Le Poing de l'Ombre", "§5The Shadow's Fist") // Translated PaP name
            : getTranslatedMessage("§7USP45", "§7USP45"); // USP45 name likely same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§ePistolet tactique moderne, précis et fiable.", "§eModern tactical pistol, precise and reliable.")));
        if (upgraded) {
