...
public class SpringfieldWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO = 5;
    private static final int MAX_RESERVE = MAX_AMMO * 8; // Rejoint la Kar98k pour la réserve
    private static final int BOLT_CYCLE_TIME = 38; // Légèrement plus rapide que la Kar98k
    private static final int RELOAD_TIME = 48; // Légèrement plus rapide que la Kar98k
    
    private static final float WEAPON_DAMAGE = 6.5f; // Légèrement plus que la Kar98k
    private static final float PAP_BONUS_DAMAGE = 4.0f; // Bonus PaP légèrement supérieur
    
    private static final float BASE_HEADSHOT_DAMAGE = 5.2f; // Multiplicateur headshot de base légèrement meilleur
    private static final float PAP_HEADSHOT_BONUS = 2.1f; 

    private static final float BASE_PROJECTILE_VELOCITY = 5.2f; // Vitesse de projectile légèrement supérieure
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.15f; 
    private static final float BASE_PROJECTILE_SPREAD = 0.008f; // Légèrement plus précis que la Kar98k
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.45f; 

    private static final float BASE_RECOIL_PITCH = 2.4f; // Recul légèrement moindre que la Kar98k
    private static final float BASE_RECOIL_YAW = 0.9f;
    private static final float PAP_RECOIL_MULTIPLIER = 0.35f; 

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.SPRINGFIELD_FIRE.get(); // À définir
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.SPRINGFIELD_RELOADING.get(); // À définir
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§cL'Esprit de '76", "§cL'Esprit de '76") // Translated PaP name
            : getTranslatedMessage("§7Springfield", "§7Springfield"); // Springfield name likely same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eFusil de précision américain, puissant et maniable.", "§ePowerful and manageable American sniper rifle.")));
        if (upgraded) {
