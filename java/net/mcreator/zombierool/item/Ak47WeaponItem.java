...
public class Ak47WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 30; // Un chargeur classique de 30 balles
    private static final int MAX_RESERVE    = MAX_AMMO * 8; // Bonne réserve pour une arme polyvalente
    private static final int COOLDOWN_TICKS = 3; // Un peu plus lente que la M16A4, 3 ticks = 0.15 secondes
    private static final int RELOAD_TIME    = 60; // Temps de rechargement moyen (3.5s)

    private static final float WEAPON_DAMAGE = 7.0f; // Bon dégât de base
    private static final float PAP_BONUS_DAMAGE = 10.0f; // Bon bonus PaP, pour rester compétitive

    private static final float BASE_HEADSHOT_DAMAGE = 14.0f; // Dégâts de base du headshot, tue en un coup les zombies de base
    private static final float PAP_HEADSHOT_BONUS = 10.0f; // Bonus headshot PaP pour les manches plus tardives

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 3.2f; // Vitesse de projectile légèrement supérieure à la M16
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.15f; // Multiplicateur de vitesse pour PaP
    private static final float BASE_PROJECTILE_SPREAD = 0.8f; // Dispersion plus élevée (moins précise)
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.6f; // Amélioration notable de la précision avec PaP

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 1.0f; // Recul en haut/bas prononcé
    private static final float BASE_RECOIL_YAW = 0.6f;   // Recul gauche/droite perceptible
    private static final float PAP_RECOIL_MULTIPLIER = 0.5f; // Réduction significative du recul avec PaP

    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.AK47_FIRE.get(); // À créer : son de tir AK-47
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.AK47_RELOADING.get(); // À créer : son de rechargement AK-47
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§cApocalypse Écarlate", "§cScarlet Apocalypse") // Translated PaP name
            : getTranslatedMessage("§4AK-47", "§4AK-47"); // AK-47 name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eFusil d'assaut automatique lourd", "§eHeavy Automatic Assault Rifle")));
        tips.add(Component.literal(getTranslatedMessage("§aUn classique fiable, idéal pour les combats rapprochés et à moyenne portée.", "§aA reliable classic, ideal for close to medium-range combat."))); // Added descriptive text
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : Apocalypse Écarlate", "§dPaP Name: Scarlet Apocalypse"))); // Specific PaP name - now translated
            tips.add(Component.literal(getTranslatedMessage(
                "§dBonus dégâts PaP : " + String.format("%.1f", PAP_BONUS_DAMAGE), // Corrected to PAP_BONUS_DAMAGE
                "§dPaP Damage Bonus: " + String.format("%.1f", PAP_BONUS_DAMAGE)
            )));
            tips.add(Component.literal(getTranslatedMessage("§dDispersion et recul grandement réduits.", "§dGreatly reduced spread and recoil."))); // Added descriptive text
        }
    }
