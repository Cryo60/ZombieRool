...
public class ScarHWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 20; // Chargeur plus petit pour une arme puissante
    private static final int MAX_RESERVE    = MAX_AMMO * 6; // Réserve modérée
    private static final int COOLDOWN_TICKS = 3; // Cadence de tir plus lente, 4 ticks = 0.2 secondes
    private static final int RELOAD_TIME    = 70; // Rechargement un peu plus lent (3.5s)

    private static final float WEAPON_DAMAGE = 14.0f; // TRÈS hauts dégâts de base par balle
    private static final float PAP_BONUS_DAMAGE = 15.0f; // Bonus PaP massif pour une puissance dévastatrice

    private static final float BASE_HEADSHOT_DAMAGE = 27.0f; // One-shot headshot jusqu'à la vague 11 (26 cœurs)
    private static final float PAP_HEADSHOT_BONUS = 15.0f; // Bonus headshot PaP massif

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 3.8f; // Très haute vitesse de projectile
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Bon gain de vitesse
    private static final float BASE_PROJECTILE_SPREAD = 0.3f; // EXCELLENTE précision de base
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.3f; // Précision quasi parfaite avec PaP

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 1.2f; // Recul en haut/bas prononcé
    private static final float BASE_RECOIL_YAW = 0.7f;   // Recul gauche/droite notable
    private static final float PAP_RECOIL_MULTIPLIER = 0.5f; // Réduction significative du recul avec PaP

    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.SCARH_FIRE.get(); // À créer : son de tir SCAR-H
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.SCARH_RELOADING.get(); // À créer : son de rechargement SCAR-H
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§5Le Châtiment du Destin", "§5The Scourge of Destiny") // Translated PaP name
            : getTranslatedMessage("§bSCAR-H", "§bSCAR-H"); // SCAR-H name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eFusil d'assaut lourd et puissant", "§eHeavy and Powerful Assault Rifle")));
        tips.add(Component.literal(getTranslatedMessage("§aDégâts élevés et précision mortelle à longue portée.", "§aHigh damage and deadly accuracy at long range."))); // Added descriptive text
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : Le Châtiment du Destin", "§dPaP Name: The Scourge of Destiny"))); // Specific PaP name - now translated
