...
public class MauserC96WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon, IHandgunWeapon {

    private static final int MAX_AMMO = 10; // Plus que le M1911 (8), pour sa petite amélioration
    private static final int MAX_RESERVE = MAX_AMMO * 8; // Réserve légèrement moins généreuse que le M1911 proportionnellement
    private static final int COOLDOWN_TICKS = 4; // Un peu plus rapide que le M1911 (5 ticks)
    private static final int RELOAD_TIME = 35; // Un peu plus long que le M1911 (30 ticks) car le chargeur est plus grand
    
    private static final float WEAPON_DAMAGE = 0.5f; // Légèrement supérieur au M1911 (0.4f)
    private static final float PAP_BONUS_DAMAGE = 3.5f; // Bonus un peu plus grand que le M1911 (3.0f)
    
    private static final float BASE_HEADSHOT_DAMAGE = 1.2f; // Multiplicateur de base légèrement meilleur que le M1911 (1.0f)
    private static final float PAP_HEADSHOT_BONUS = 2.5f; // Bonus de headshot PaP légèrement meilleur que le M1911 (2.0f)

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 2.2f; // Légèrement plus rapide que le M1911 (2.0f)
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.15f; // Multiplicateur un peu plus grand
    private static final float BASE_PROJECTILE_SPREAD = 0.9f; // Légèrement plus précis que le M1911 (1.0f)
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.65f; // Multiplicateur de dispersion PaP un peu meilleur

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 0.8f; // Légèrement moins de recul que le M1911 (0.9f)
    private static final float BASE_RECOIL_YAW = 0.4f;   // Légèrement moins de recul que le M1911 (0.5f)
    private static final float PAP_RECOIL_MULTIPLIER = 0.75f; // Multiplicateur de recul PaP un peu meilleur

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.MAUSER_FIRE.get();
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Peut être personnalisé si besoin
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.MAUSER_RELOADING.get();
    private static final SoundEvent DRY_FIRE_SOUND = ZombieroolModSounds.RIFLE_DRY.get(); // Peut être personnalisé si besoin
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§eLe Balafré", "§eThe Scarred One") // Nom PaP : "Le Balafré" -> "The Scarred One"
            : getTranslatedMessage("§7Mauser C96", "§7Mauser C96"); // Mauser C96 likely same in both languages
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§ePistolet de départ. Préférable à la M1911.", "§eStarting pistol. Preferable to the M1911.")));
        if (upgraded) {
