...
public class VandalWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 25; // Très grand chargeur
    private static final int MAX_RESERVE    = MAX_AMMO * 18; // Réserve colossale
    private static final int COOLDOWN_TICKS = 2; // Cadence de tir très élevée
    private static final int RELOAD_TIME    = 40; // Rechargement ultra rapide

    private static final float WEAPON_DAMAGE = 18.0f; // Dégâts de base excellents
    private static final float PAP_BONUS_DAMAGE = 25.0f; // Bonus Pack-a-Punch absolument monstrueux

    private static final float BASE_HEADSHOT_DAMAGE = 30.0f; // Multiplicateur de headshot dévastateur
    private static final float PAP_HEADSHOT_BONUS = 20.0f;    // Bonus headshot PaP incroyable

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 4.0f; // Vitesse de projectile maximale
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.3f; // Vitesse encore plus folle après PaP
    private static final float BASE_PROJECTILE_SPREAD = 0.15f; // Précision quasi parfaite
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.2f; // Précision absolue après PaP (0.15 * 0.2 = 0.03)

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 0.3f; // Recul vertical minimal
    private static final float BASE_RECOIL_YAW = 0.1f;   // Recul horizontal quasi inexistant
    private static final float PAP_RECOIL_MULTIPLIER = 0.3f; // Recul presque annulé après PaP


    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.VANDAL_FIRE.get(); // Utilise VANDAL_FIRE
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Son de tir amélioré générique
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.VANDAL_RELOADING.get(); // Utilise VANDAL_RELOADING
    ...
    public VandalWeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.EPIC)); // Rareté EPIC, ou même LEGENDARY si tu as une option pour cela
    }
    ...
    // Nouveau getter pour la propriété de texture "Dragon" (liée à isPackAPunched)
    public boolean isDragonVandal(ItemStack stack) {
        return isPackAPunched(stack); // La texture "Dragon" est active si l'arme est Pack-a-Punchée
    }
    ...
    public void initializeIfNeeded(ItemStack stack) {
        ...
        // Pas de logique Black Ice ici, car la Vandal Dragon est liée à PaP
    }
    ...
    public Component getName(ItemStack stack) {
        ...
        boolean isDragon = isDragonVandal(stack); // Vérifie si c'est la Vandal Dragon

        String nameColor = upgraded ? "§6" : "§2"; // Oranged pour PaP, vert pour normal (couleur de base)
        String baseName = getTranslatedMessage("Vandal", "Vandal"); // Vandal reste Vandal, mais traduisible
        String papName = getTranslatedMessage("Le Fléau du Dragon", "The Dragon's Bane"); // Nom PaP pour la Vandal Dragon

        if (isDragon) {
            nameColor = "§c"; // Rouge ardent pour le Dragon
        }
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        boolean isDragon = isDragonVandal(stack);

        tips.add(Component.literal(getTranslatedMessage("§eFusil d'assaut d'élite | Le cauchemar des morts-vivants", "§eElite Assault Rifle | The Undead's Nightmare")));
        if (isDragon) {
            tips.add(Component.literal(getTranslatedMessage("§cLa puissance draconière à l'état pur !", "§cPure Draconic Power!")));
        }
        if (upgraded) {
            ...
            tips.add(Component.literal(getTranslatedMessage("§dPrécision et recul quasiment annulés.", "§dAccuracy and recoil almost nullified."))); // Ajouté pour être cohérent
        }
    }

    @Override
    public boolean isFoil(ItemStack stack) {
        // La Vandal Pack-a-Punchée (et donc Vandal Dragon) aura un effet brillant
