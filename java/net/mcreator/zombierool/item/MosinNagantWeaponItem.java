...
public class MosinNagantWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO = 5;
    private static final int MAX_RESERVE = MAX_AMMO * 6; // Un peu moins de réserve que la Kar98k
    private static final int BOLT_CYCLE_TIME = 45; // Plus lent que la Kar98k (2.25 secondes)
    private static final int RELOAD_TIME = 30; // Un peu plus long à recharger (3.25 secondes)
    
    private static final float WEAPON_DAMAGE = 9.0f; // Légèrement moins de dégâts de base que la Kar98k
    private static final float PAP_BONUS_DAMAGE = 12.0f; // Bonus PaP un peu plus faible

    // Le Mosin sera moins efficace pour les headshots "purs" mais aura un bon multiplicateur de base pour compenser ses dégâts plus faibles.
    private static final float BASE_HEADSHOT_DAMAGE = 4.5f; // Multiplicateur de base
    private static final float PAP_HEADSHOT_BONUS = 1.8f; // Multiplicateur additionnel PaP

    private static final float BASE_PROJECTILE_VELOCITY = 4.5f; // Vitesse de projectile légèrement plus faible
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f;
    private static final float BASE_PROJECTILE_SPREAD = 0.02f; // Légèrement moins précis de base
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.6f; // Amélioration de précision un peu moins bonne

    private static final float BASE_RECOIL_PITCH = 2.8f; // Plus de recul vertical
    private static final float BASE_RECOIL_YAW = 1.2f; // Plus de recul horizontal
    private static final float PAP_RECOIL_MULTIPLIER = 0.5f; // Amélioration du recul un peu moins bonne

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.MOSIN_FIRE.get();
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.MOSIN_RELOADING.get();
    ...
    private static final String TAG_BOLT_CYCLE_TIMER = "BoltCycleTimer";
    ...
    public void initializeIfNeeded(ItemStack stack) {
        ...
        if (!tag.contains(TAG_BOLT_CYCLE_TIMER)) {
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity ent, int slot, boolean sel) {
        ...
        int boltCycleTimer = getBoltCycleTimer(stack);
        ...
        if (boltCycleTimer > 0) {
            ...
            setBoltCycleTimer(stack, --boltCycleTimer);
        }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§6Le Vieux Fidèle", "§6The Old Reliable") // Translated PaP name
            : getTranslatedMessage("§7Mosin Nagant", "§7Mosin Nagant"); // Mosin Nagant name likely same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eFusil de précision à verrou robuste.", "§eRobust bolt-action sniper rifle.")));
        if (upgraded) {
