...
public class AUGWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 30;
    private static final int MAX_RESERVE    = MAX_AMMO * 8; // Plus de réserves pour une arme puissante
    private static final int COOLDOWN_TICKS = 4; // Cadence de tir modérée, entre un fusil d'assaut et un pistolet-mitrailleur (0.2s)
         
    private static final int RELOAD_TIME    = 70; // ticks (3.5s) - un peu plus long pour équilibrer
         
    private static final float WEAPON_DAMAGE = 9.0f; // Dégâts de base élevés pour un 8/10
    private static final float PAP_BONUS_DAMAGE = 12.0f; // Très bon bonus PaP
         
    private static final float BASE_HEADSHOT_DAMAGE = 20.0f; // Gros dégâts de base du headshot
    private static final float PAP_HEADSHOT_BONUS = 15.0f;    // Excellent bonus headshot si PaP

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 3.5f; // Vitesse de projectile rapide
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.15f; // Bon multiplicateur de vitesse pour PaP
    private static final float BASE_PROJECTILE_SPREAD = 0.3f; // Très précis de base
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.6f; // Encore plus précis avec PaP

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 0.8f; // Recul vertical notable
    private static final float BASE_RECOIL_YAW = 0.4f;   // Recul horizontal modéré
    private static final float PAP_RECOIL_MULTIPLIER = 0.5f; // Recul bien réduit avec PaP


    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.AUG_FIRE.get(); // Assure-toi d'avoir ce son
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.AUG_RELOADING.get(); // Son de rechargement générique pour fusil
    ...
    public AUGWeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.RARE)); // Rareté RARE pour une arme puissante
    }
    ...
    // Corrected implementation for getReloadTimer and setReloadTimer
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§5AEG", "§5Automatic Energy Gun") // Translated PaP name
            : getTranslatedMessage("§aAUG", "§aAUG"); // AUG name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        ...
        tips.add(Component.literal(getTranslatedMessage("§aPolyvalent et précis, avec une bonne cadence de tir.", "§aVersatile and accurate, with a good rate of fire."))); // Added descriptive text
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : AEG", "§dPaP Name: Automatic Energy Gun"))); // Specific PaP name - now translated
