...
public class BizonWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 64; // Très grand chargeur, signature de la Bizon
    private static final int MAX_RESERVE    = MAX_AMMO * 4; // Réserve modérée compte tenu de la capacité
    private static final int COOLDOWN_TICKS = 4; // Cadence de tir modérée (0.2 secondes)
    private static final int RELOAD_TIME    = 70; // Recharge plus longue (3.5s)
           
    private static final float WEAPON_DAMAGE = 5.0f; // Dégâts de base corrects, entre MP5 et UMP45
    private static final float PAP_BONUS_DAMAGE = 7.0f; // Bon bonus PaP pour la maintenir viable
           
    private static final float BASE_HEADSHOT_DAMAGE = 1.6f; // Multiplicateur de dégâts pour headshot (1.6x du WEAPON_DAMAGE)
    private static final float PAP_HEADSHOT_BONUS = 6.0f;    // Bonus de dégâts du headshot si PaP
    // Dégâts de base du headshot : WEAPON_DAMAGE * BASE_HEADSHOT_DAMAGE_MULTIPLIER = 5.0 * 1.6 = 8.0
    // Dégâts PaP du headshot : (WEAPON_DAMAGE + PAP_BONUS_DAMAGE) * BASE_HEADSHOT_DAMAGE_MULTIPLIER + PAP_HEADSHOT_BONUS = (5.0 + 7.0) * 1.6 + 6.0 = 12.0 * 1.6 + 6.0 = 19.2 + 6.0 = 25.2

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 2.9f; // Vitesse de projectile standard
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Léger boost PaP
    private static final float BASE_PROJECTILE_SPREAD = 0.9f; // Bonne précision de base pour une SMG
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.6f; // Bonne amélioration de la dispersion PaP
    
    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 0.4f; // Faible recul vertical pour une SMG
    private static final float BASE_RECOIL_YAW = 0.4f;   // Faible recul horizontal, très contrôlable
    private static final float PAP_RECOIL_MULTIPLIER = 0.5f; // Très bon gain sur le recul PaP

    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.BIZON_FIRE.get(); // Placeholders pour les sons
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.BIZON_RELOADING.get();
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§dBizon-1000", "§dBizon-1000") // Proper noun, remains same
            : getTranslatedMessage("§2Bizon", "§2Bizon"); // Proper noun, remains same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§ePistolet-mitrailleur à haute capacité", "§eHigh Capacity Submachine Gun")));
        tips.add(Component.literal(getTranslatedMessage("§aUn pistolet-mitrailleur avec un très grand chargeur, idéal pour le contrôle de foule.", "§aA submachine gun with a very large magazine, ideal for crowd control."))); // Added descriptive text
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : Bizon-1000", "§dPaP Name: Bizon-1000"))); // Specific PaP name - now translated
            ...
            tips.add(Component.literal(getTranslatedMessage("§dPrécision et recul grandement améliorés.", "§dGreatly improved accuracy and recoil."))); // Added descriptive text
        }
    }
