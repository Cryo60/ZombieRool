...
public class DMRWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    // --- Statistiques de l'arme (5/10) ---
    private static final int MAX_AMMO       = 15; // Moins de balles, accent sur la précision
    private static final int MAX_RESERVE    = MAX_AMMO * 8; // Bonnes réserves pour compenser
    private static final int COOLDOWN_TICKS = 8; // Cadence de tir plus lente, tir semi-automatique
    private static final int RELOAD_TIME    = 45; // Temps de rechargement légèrement plus long

    private static final float WEAPON_DAMAGE = 9.0f; // Dégâts par coup élevés, comparables au Magnum
    private static final float PAP_BONUS_DAMAGE = 4.5f; // Très bon bonus pour le Pack-a-Punch

    private static final float BASE_HEADSHOT_DAMAGE = 2.0f; // Excellent multiplicateur de headshot
    private static final float PAP_HEADSHOT_BONUS = 3.2f;    // Très gros bonus si PaP

    // Bullet Parameters
    private static final float BASE_PROJECTILE_VELOCITY = 2.5f; // Projectile très rapide
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Vitesse augmentée
    private static final float BASE_PROJECTILE_SPREAD = 0.5f; // Très peu de dispersion
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.5f; // Précision quasi parfaite avec PaP

    // Recoil Parameters
    private static final float BASE_RECOIL_PITCH = 1.0f; // Recul vertical modéré mais présent
    private static final float BASE_RECOIL_YAW = 0.3f;   // Très faible recul horizontal
    private static final float PAP_RECOIL_MULTIPLIER = 0.6f; // Recul significativement réduit avec PaP

    // --- Sons de l'arme (adaptés à tes sons génériques) ---
    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.DMR_FIRE.get(); // Son spécifique au DMR
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Son générique
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.DMR_RELOADING.get(); // Son spécifique au DMR
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§dDistanceur", "§dDistancer") // Translated PaP name
            : getTranslatedMessage("§aDMR", "§aDMR"); // Base name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eFusil de précision tactique, idéal pour l'élimination à distance.", "§eTactical precision rifle, ideal for long-range elimination."))); // Description
        if (upgraded) {
