...
public class Kar98kWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO = 5;
    private static final int MAX_RESERVE = MAX_AMMO * 8;
    // COOLDOWN_TICKS est la durée de l'animation de "rechargement/verrouillage" après chaque tir
    private static final int BOLT_CYCLE_TIME = 30; // 2 secondes complètes pour l'animation du cycle du verrou
    private static final int RELOAD_TIME = 40; // 2.5 secondes complètes pour le rechargement complet du chargeur
    
    private static final float WEAPON_DAMAGE = 4.0f;
    private static final float PAP_BONUS_DAMAGE = 5.0f;
    
    private static final float BASE_HEADSHOT_DAMAGE = 8.0f;
    private static final float PAP_HEADSHOT_BONUS = 2.0f;

    private static final float BASE_PROJECTILE_VELOCITY = 5.0f;
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f;
    private static final float BASE_PROJECTILE_SPREAD = 0.01f;
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.5f;

    private static final float BASE_RECOIL_PITCH = 2.5f;
    private static final float BASE_RECOIL_YAW = 1.0f;
    private static final float PAP_RECOIL_MULTIPLIER = 0.4f;

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.KAR98K_FIRE.get();
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.KAR98K_RELOADING.get();
    ...
    private static final String TAG_BOLT_CYCLE_TIMER = "BoltCycleTimer"; // Nouveau tag pour le timer du cycle de culasse
    ...
    // Nouveaux getters/setters pour le timer du cycle de culasse
    public int getBoltCycleTimer(ItemStack s) { return getOrCreateTag(s).getInt(TAG_BOLT_CYCLE_TIMER); }
    public void setBoltCycleTimer(ItemStack s, int t) { getOrCreateTag(s).putInt(TAG_BOLT_CYCLE_TIMER, t); }
    ...
    public void initializeIfNeeded(ItemStack stack) {
        ...
        if (tag == null || !tag.contains("Ammo")) {
            ...
            setBoltCycleTimer(stack, 0); // Initialiser le nouveau timer
            tag = stack.getTag();
        }
        ...
        if (!tag.contains(TAG_BOLT_CYCLE_TIMER)) { // S'assurer que le tag existe
            tag.putInt(TAG_BOLT_CYCLE_TIMER, 0);
        }
    }
    ...
    public void startReload(ItemStack stack, Player player) {
        ...
        // Ne lance le rechargement que si l'arme n'est pas déjà en cours de rechargement ET qu'il y a des balles en réserve
        // Et que le chargeur n'est pas déjà plein.
        if (getReloadTimer(stack) == 0 && getAmmo(stack) < MAX_AMMO && getReserve(stack) > 0) {
            ...
            if (!player.level().isClientSide) {
                setReloadTimer(stack, reloadTime); // Démarre le VRAI timer de rechargement
                setBoltCycleTimer(stack, 0); // S'assurer que le cycle de culasse s'arrête si un rechargement est lancé
                player.level().playSound(null, player.getX(), player.getY(), player.getZ(), RELOAD_SOUND, SoundSource.PLAYERS, 1f, pitch); 

                // Cherry Cola effects are only tied to actual reloading (full magazine reload)
    ...
    // Renvoie l'animation d'utilisation : NONE si en rechargement (complet ou culasse), BOW pour tirer
    @Override public UseAnim getUseAnimation(ItemStack s) { 
        return (getReloadTimer(s) > 0 || getBoltCycleTimer(s) > 0) ? UseAnim.NONE : UseAnim.BOW; 
    }
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        // Si l'arme est en cours de rechargement complet, on ne peut pas tirer
        if (getReloadTimer(stack) > 0) {
            return;
        }

        // Si le cycle de culasse est en cours, on ne peut pas tirer
        if (getBoltCycleTimer(stack) > 0) {
            return;
        }

        // Si plus de balles dans le chargeur mais en réserve, lance le rechargement complet
        if (getAmmo(stack) == 0 && getReserve(stack) > 0) {
            startReload(stack, player);
            return;
        }
        
        // Si plus de balles du tout (clic à vide)
        if (getAmmo(stack) == 0 && getReserve(stack) == 0) {
        ...
        if (getAmmo(stack) > 0) {
            ...
            // Après le tir, si ce n'est pas le dernier coup, déclencher le timer du cycle de culasse.
            // Cela va forcer l'arme à utiliser l'animation "NONE" (donc l'animation de rechargement si elle est configurée comme telle visuellement)
            // pendant la durée du BOLT_CYCLE_TIME.
            if (getAmmo(stack) > 0) { 
                setBoltCycleTimer(stack, BOLT_CYCLE_TIME);
                // Important : NE PAS jouer le son de rechargement ici, car ce n'est pas un rechargement complet.
            }
        }
    }
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity ent, int slot, boolean sel) {
        ...
        int boltCycleTimer = getBoltCycleTimer(stack); // Récupère le timer du cycle de culasse
    
        if (level.isClientSide) {
            int expectedReloadTimeForCherry = RELOAD_TIME;
            if (p.hasEffect(ZombieroolModMobEffects.PERKS_EFFECT_SPEED_COLA.get())) {
                expectedReloadTimeForCherry /= 2;
            }
            // L'animation Cherry Cola ne se déclenche que si le timer correspond à un rechargement complet (RELOAD_TIME)
            if (reloadTimer == expectedReloadTimeForCherry && p.hasEffect(ZombieroolModMobEffects.PERKS_EFFECT_CHERRY.get())) {
                CherryReloadAnimationHandler.startCherryAnimation(null);
            }
            return;
        }
    
        // Logique côté serveur pour le timer de rechargement complet
        if (reloadTimer > 0) {
            if (!sel || !p.getItemInHand(InteractionHand.MAIN_HAND).equals(stack)) {
                setReloadTimer(stack, 0); // Interrompre le timer si l'arme n'est plus en main
                return; 
            }
        ...
        // Logique côté serveur pour le timer du cycle de culasse
        if (boltCycleTimer > 0) {
            if (!sel || !p.getItemInHand(InteractionHand.MAIN_HAND).equals(stack)) {
                setBoltCycleTimer(stack, 0); // Interrompre si l'arme n'est plus en main
                return;
            }
            setBoltCycleTimer(stack, --boltCycleTimer); // Décrémenter le timer
        }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§eLa Précision Allemande", "§eThe German Precision") // Translated PaP name
            : getTranslatedMessage("§7Kar98k", "§7Kar98k"); // Kar98k name likely same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eFusil à verrou. Lent mais dévastateur.", "§eBolt-action rifle. Slow but devastating.")));
        if (upgraded) {
