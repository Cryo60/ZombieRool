...
public class ChinaLakeWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    // --- Statistiques de l'arme (8/10) --- Légèrement moins puissant que le RPG, plus de munitions
    private static final int MAX_AMMO       = 2; // Deux tirs par chargeur
    private static final int MAX_RESERVE    = MAX_AMMO * 6; // Plus de grenades en réserve que le RPG de roquettes
    private static final int COOLDOWN_TICKS = 30; // Temps entre les tirs
    private static final int RELOAD_TIME    = 60; // Temps de rechargement moyen

    private static final float WEAPON_DAMAGE = 15.0f; // Dégâts directs corrects
    private static final float PAP_BONUS_DAMAGE = 25.0f; // Gros bonus pour le Pack-a-Punch

    private static final float BASE_HEADSHOT_DAMAGE = 0.5f; // Headshots très peu pertinents
    private static final float PAP_HEADSHOT_BONUS = 1.0f;    // Bonus headshot insignifiant

    // Projectile Parameters (lent, mais plus rapide que le RPG)
    private static final float BASE_PROJECTILE_VELOCITY = 1.2f; // Projectile lent, mais suffisant
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.15f; // Légèrement plus rapide
    private static final float BASE_PROJECTILE_SPREAD = 0.7f; // Précision plus faible qu'un RPG, mais c'est une grenade
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 1.0f; // Pas de changement

    // Recoil Parameters (recul important pour une arme à 2 coups, mais gérable)
    private static final float BASE_RECOIL_PITCH = 3.0f; // Recul vertical important
    private static final float BASE_RECOIL_YAW = 1.5f;   // Recul horizontal notable
    private static final float PAP_RECOIL_MULTIPLIER = 0.8f; // Recul un peu réduit avec PaP

    // --- Explosion Parameters ---
    public static final String TAG_IS_CHINALAKE_PROJECTILE = "zombierool:is_chinalake_projectile"; // Nouveau tag pour l'ArrowImpactHandler
    public static final float BASE_EXPLOSION_RADIUS = 4.0f; // Rayon d'explosion légèrement plus petit que le RPG
    public static final float PAP_EXPLOSION_RADIUS_MULTIPLIER = 1.2f; // Rayon un peu plus grand avec PaP
    public static final float BASE_EXPLOSION_DAMAGE_MULTIPLIER = 1.8f; // Multiplicateur de dégâts de l'explosion
    public static final float PAP_EXPLOSION_DAMAGE_MULTIPLIER = 3.5f; // Très gros multiplicateur pour l'explosion PaP

    // --- Sons de l'arme ---
    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.GRANADE_LAUNCHER_FIRE.get(); // À adapter si tu as un son spécifique
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Son générique
    // Fix: Corrected typo GRANADE_LAUNCHER_RELOADING to GRANADE_LAUNCER_RELOADING and added .get()
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.GRANADE_LAUNCER_RELOADING.get(); // À adapter
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        if (getAmmo(stack) > 0) {
            ...
            // Marquer cette flèche comme un projectile du China Lake pour l'ArrowImpactHandler
            arrow.getPersistentData().putBoolean(TAG_IS_CHINALAKE_PROJECTILE, true);
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§6Talon du Dragon", "§6Dragon's Talon") // Translated PaP name
            : getTranslatedMessage("§eChina Lake", "§eChina Lake"); // Base name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eLance-grenades polyvalent, idéal pour le contrôle de zone.", "§eVersatile grenade launcher, ideal for area control."))); // Description
        if (upgraded) {
            ...
            tips.add(Component.literal(getTranslatedMessage(
                "§6Bonus dégâts AoE PaP : " + String.format("%.1f", getExplosionDamageMultiplier(stack)) + "x",
                "§6PaP AoE Damage Bonus: " + String.format("%.1f", getExplosionDamageMultiplier(stack)) + "x"
            )));
        }
    }
