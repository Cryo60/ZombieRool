...
public class P90WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 50; // Grande capacité de chargeur, typique de la P90
    private static final int MAX_RESERVE    = MAX_AMMO * 6; // Bonne réserve pour le tir soutenu
    private static final int COOLDOWN_TICKS = 2; // Très haute cadence de tir (0.1 secondes, comme la M16A4 mais en auto)
    private static final int RELOAD_TIME    = 65; // Recharge modérée (3.25s)
           
    private static final float WEAPON_DAMAGE = 4.5f; // Dégâts de base corrects, entre MP5 et UMP45
    private static final float PAP_BONUS_DAMAGE = 7.5f; // Très bon bonus PaP, la rendant redoutable
           
    private static final float BASE_HEADSHOT_DAMAGE = 1.7f; // Multiplicateur de dégâts pour headshot (1.7x du WEAPON_DAMAGE)
    private static final float PAP_HEADSHOT_BONUS = 7.0f;    // Bonus de dégâts du headshot si PaP
    // Dégâts de base du headshot : WEAPON_DAMAGE * BASE_HEADSHOT_DAMAGE_MULTIPLIER = 4.5 * 1.7 = 7.65
    // Dégâts PaP du headshot : (WEAPON_DAMAGE + PAP_BONUS_DAMAGE) * BASE_HEADSHOT_DAMAGE_MULTIPLIER + PAP_HEADSHOT_BONUS = (4.5 + 7.5) * 1.7 + 7.0 = 12.0 * 1.7 + 7.0 = 20.4 + 7.0 = 27.4

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 3.1f; // Bonne vitesse de projectile
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.2f; // Très bon boost PaP
    private static final float BASE_PROJECTILE_SPREAD = 0.8f; // Excellente précision de base
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.5f; // Très bonne amélioration de la dispersion PaP
    
    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 0.4f; // Faible recul vertical
    private static final float BASE_RECOIL_YAW = 0.3f;   // Très faible recul horizontal, très contrôlable
    private static final float PAP_RECOIL_MULTIPLIER = 0.4f; // Excellent gain sur le recul PaP

    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.P90_FIRE.get(); // Placeholders pour les sons
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.P90_RELOADING.get();
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§dThe Waffle", "§dThe Waffle") // Proper noun, remains same
            : getTranslatedMessage("§2P90", "§2P90"); // Proper noun, remains same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§ePistolet-mitrailleur à cadence élevée", "§eHigh Rate of Fire Submachine Gun")));
        tips.add(Component.literal(getTranslatedMessage("§aUn pistolet-mitrailleur avec une très haute cadence de tir et une grande précision.", "§aA submachine gun with a very high rate of fire and great accuracy."))); // Added descriptive text
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : The Waffle", "§dPaP Name: The Waffle"))); // Specific PaP name - now translated
