...
import net.minecraft.util.RandomSource; // Nécessaire si tu utilises player.getRandom() pour le recul/spread
// import java.util.Random; // Assure-toi que cette ligne n'est PAS là
...
public class Mp40WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 40;   // 40 balles par chargeur (Augmenté de 32 à 40)
    private static final int MAX_RESERVE    = 240;  // 240 balles en réserve (6 chargeurs) (Augmenté de 192 à 240)
    private static final int COOLDOWN_TICKS = 2;    // Inchangé (déjà très rapide, l'augmenter rendrait le buff trop extrême ou changerait la nature de l'arme)
    private static final int RELOAD_TIME    = 30;   // 1.5 secondes pour recharger (Réduit de 40 à 30)

    private static final float WEAPON_DAMAGE        = 13.0f;  // Dégâts de base augmentés (Ancien : 10.0f)
    private static final float PAP_BONUS_DAMAGE     = 12.0f;  // Bonus PaP très fort (Ancien : 10.0f)

    private static final float BASE_HEADSHOT_DAMAGE = 2.5f;   // Bonus de headshot très généreux (Ancien : 2.0f)
    private static final float PAP_HEADSHOT_BONUS   = 1.5f;   // Bonus supplémentaire au PaP très élevé (Ancien : 1.0f)

    // Paramètres de tir (projectile invisible)
    private static final float BASE_PROJECTILE_VELOCITY = 3.5f; // Vitesse des balles augmentée (Ancien : 3.0f)
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.3f; // Encore plus rapide après PaP (Ancien : 1.2f)
    private static final float BASE_PROJECTILE_SPREAD = 1.0f;    // Dispersion légèrement réduite (Ancien : 1.2f)
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.5f; // Très bonne réduction de dispersion avec PaP (Ancien : 0.6f)

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 0.6f; // Recul vertical réduit (Ancien : 0.8f)
    private static final float BASE_RECOIL_YAW = 0.08f;   // Léger recul latéral encore réduit (Ancien : 0.1f)
    private static final float PAP_RECOIL_MULTIPLIER = 0.5f; // Recul fortement réduit avec PaP (Ancien : 0.6f)

    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.MP40_FIRE.get(); // Tu devras créer ce son
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Et celui-ci
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.MP40_RELOADING.get(); // Et celui-ci
    ...
    public Mp40WeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.COMMON)); // Rareté commune pour cette arme de début de partie
    }
            ...
            if (!player.level().isClientSide) {
                ...
                if (player.hasEffect(ZombieroolModMobEffects.PERKS_EFFECT_CHERRY.get())) {
                    ...
                    for (LivingEntity target : entities) {
                        // Condition originale incluant Monster, TamableAnimal (non-tame), et tout ce qui n'est ni Animal ni TamableAnimal.
                        // Simplifié ici pour cibler principalement les monstres et créatures hostiles/neutres non-tame.
                        // Assurez-vous que votre logique de cible est cohérente avec votre intention de jeu.
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        if (getAmmo(stack) > 0) {
            ...
            // Ces deux tags sont cruciaux pour le système de hitscan via projectile invisible.
            // "zombierool:invisible" et "zombierool:small" (si tu l'utilises pour une hitbox plus petite)
            arrow.getPersistentData().putBoolean("zombierool:invisible", true);
            arrow.getPersistentData().putBoolean("zombierool:small", true); // Garde ce tag si tu l'utilises ailleurs
            arrow.setInvisible(true); // Rend l'entité invisible
            arrow.setBaseDamage(0.0D); // Les dégâts sont gérés par ton propre système (ex: événement on_projectile_hits_entity)
            ...
            if (player instanceof ServerPlayer serverPlayer) {
                ...
                // Le recul latéral doit être aléatoire gauche/droite
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity ent, int slot, boolean sel) {
        ...
        if (t > 0) {
            if (!sel || !p.getItemInHand(InteractionHand.MAIN_HAND).equals(stack)) {
                setReloadTimer(stack, 0);
                // System.out.println("DEBUG: Reload interrupted for " + stack.getDisplayName().getString() + " (not selected or wrong item). Timer reset to 0."); // Optionnel
                return; 
            }
            setReloadTimer(stack, --t);
            if (t <= 0) {
                finishReload(stack, p, level);
                // System.out.println("DEBUG: Reload finished for " + stack.getDisplayName().getString() + "."); // Optionnel
            }
        }
        ...
        if (sel && !wasEquippedPreviously) {
            if (p.getItemInHand(InteractionHand.MAIN_HAND).equals(stack)) {
                ...
                // System.out.println("DEBUG: Playing WEAPON_IN_HAND_SOUND for " + stack.getDisplayName().getString()); // Optionnel
            }
            tag.putBoolean(TAG_EQUIPPED_PREVIOUSLY, true);
        } else if (!sel && wasEquippedPreviously) {
            tag.putBoolean(TAG_EQUIPPED_PREVIOUSLY, false);
            // System.out.println("DEBUG: " + stack.getDisplayName().getString() + " unequipped. Resetting TAG_EQUIPPED_PREVIOUSLY."); // Optionnel
        }
    }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§4Papa-Pistolet", "§4Papa-Pistol") // Translated PaP name
            : getTranslatedMessage("§fMP40", "§fMP40"); // MP40 name likely same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§l§ePistolet-Mitrailleur", "§l§eSubmachine Gun")));
        tips.add(Component.literal(getTranslatedMessage("§aPolyvalent et fiable pour les premières vagues.", "§aVersatile and reliable for early waves.")));
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : Papa-Pistolet", "§dPaP Name: Papa-Pistol"))); // Specific PaP name
            ...
            tips.add(Component.literal(getTranslatedMessage("§dDispersion réduite.", "§dReduced spread.")));
            tips.add(Component.literal(getTranslatedMessage("§dRecul réduit.", "§dReduced recoil.")));
        }
    }
