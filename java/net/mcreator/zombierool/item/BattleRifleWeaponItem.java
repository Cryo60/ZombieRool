...
public class BattleRifleWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO = 24; // Un chargeur plus grand qu'un pistolet, adapté à un fusil
    private static final int MAX_RESERVE = MAX_AMMO * 6; // Bonne réserve de munitions
    private static final int BURST_BULLETS = 4; // Tire en rafale de 4 balles
    private static final int BURST_INTERVAL_TICKS = 2; // Temps entre chaque balle de la rafale (plus lent qu'un pistolet, plus contrôlable)
    private static final int BURST_COOLDOWN_TICKS = 15; // Temps d'attente entre les rafales
    private static final int RELOAD_TIME = 45; // Temps de rechargement d'un fusil

    private static final float WEAPON_DAMAGE = 10.0f; // Dégâts par balle modérés
    private static final float PAP_BONUS_DAMAGE = 6.0f; // Bon bonus Pack-a-Punch pour monter en puissance

    private static final float BASE_HEADSHOT_DAMAGE = 2.0f; // Multiplicateur headshot standard
    private static final float PAP_HEADSHOT_BONUS = 3.5f;    // Bon bonus de dégâts PaP headshot pour le late game

    // Bullet Parameters
    private static final float BASE_PROJECTILE_VELOCITY = 2.5f; // Projectile rapide, adapté à un fusil
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f;
    private static final float BASE_PROJECTILE_SPREAD = 0.9f; // Plus précis qu'un pistolet ou un pompe
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.5f; // Bonne amélioration de la précision PaP

    // Recoil Parameters
    private static final float BASE_RECOIL_PITCH = 1.5f; // Recul vertical modéré
    private static final float BASE_RECOIL_YAW = 0.5f;   // Recul horizontal faible
    private static final float PAP_RECOIL_MULTIPLIER = 0.6f; // Recul réduit avec PaP

    // Sound References
    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.BATTLE_RIFLE_FIRE.get(); // Son de tir de fusil
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.BATTLE_RIFLE_RELOADING.get(); // Son de rechargement de fusil d'assaut
    ...
    private static final String TAG_LAST_SHOT_TICK = "LastShotTick";
    private static final String TAG_BURST_COUNTER = "BurstCounter";
    private static final String TAG_NEXT_BURST_TICK = "NextBurstTick";
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        // Only start a new burst if not reloading, not currently in a burst, and cooldown is over
        if (getReloadTimer(player.getItemInHand(hand)) == 0 && getBurstCounter(player.getItemInHand(hand)) == 0 && level.getGameTime() >= getLastShotTick(player.getItemInHand(hand)) + BURST_COOLDOWN_TICKS) {
            ...
            setNextBurstTick(player.getItemInHand(hand), level.getGameTime()); // Set next burst tick to current tick to fire immediately
        }
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        if (getReloadTimer(stack) > 0) {
            setBurstCounter(stack, 0); // Stop any ongoing burst if reload starts
            return;
        }

        long currentTick = level.getGameTime();

        // Check if a burst is active and it's time for the next bullet
        if (getBurstCounter(stack) > 0 && currentTick >= getNextBurstTick(stack)) {
            if (getAmmo(stack) == 0) { // If out of ammo in magazine
                if (getReserve(stack) > 0) { // And has reserve ammo, start reload
                ...
                setBurstCounter(stack, 0); // Stop burst regardless of reload or dry fire
                player.stopUsingItem(); // Stop using item to prevent continuous dry fire
                return;
            }
            ...
            arrow.setBaseDamage(getWeaponDamage(stack)); // Apply weapon damage directly to arrow
            ...
            setNextBurstTick(stack, currentTick + BURST_INTERVAL_TICKS); // Schedule next bullet in burst

            if (getBurstCounter(stack) == 0) { // If burst is finished
                setLastShotTick(stack, currentTick); // Set cooldown for next burst
                player.stopUsingItem(); // Stop using item to allow releasing click
            }
        } else if (getBurstCounter(stack) == 0 && player.isUsingItem() && currentTick >= getLastShotTick(stack) + BURST_COOLDOWN_TICKS) {
            // If player is holding click after a burst cooldown, start a new burst
             if (getAmmo(stack) > 0) {
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity ent, int slot, boolean sel) {
        ...
        if (t > 0) {
            if (!sel || !p.getItemInHand(InteractionHand.MAIN_HAND).equals(stack)) {
                setReloadTimer(stack, 0);
                setBurstCounter(stack, 0); // Interrupt burst if weapon is unequipped during reload
        ...
        // If player stops using item mid-burst, reset burst counter
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§cL'Avant-Garde", "§cThe Vanguard")
            : getTranslatedMessage("§fFusil de Bataille", "§fBattle Rifle");
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eFusil d'assaut semi-automatique avec tir en rafale.", "§eSemi-automatic assault rifle with burst fire.")));
        tips.add(Component.literal(getTranslatedMessage("§aPrécis et efficace contre les groupes de zombies.", "§aPrecise and effective against groups of zombies.")));
        if (upgraded) {
            ...
            tips.add(Component.literal(getTranslatedMessage(
                "§dNom PaP : L'Avant-Garde",
                "§dPaP Name: The Vanguard"
            )));
            tips.add(Component.literal(getTranslatedMessage(
