...
public class Beretta93rWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon, IHandgunWeapon {

    private static final int MAX_AMMO = 21; // Buffé : Un peu plus qu'un pistolet standard, mais moins que le Five-SeveN
    private static final int MAX_RESERVE = MAX_AMMO * 5; // Réserve modérée
    private static final int BURST_BULLETS = 3; // Tir en rafale de 3 balles
    private static final int BURST_INTERVAL_TICKS = 1; // Buffé : Temps entre chaque balle de la rafale (plus rapide)
    private static final int BURST_COOLDOWN_TICKS = 12; // Buffé : Temps d'attente entre les rafales (plus court)
    private static final int RELOAD_TIME = 35; // Rechargement un peu plus long que l'USP45/Five-SeveN
    
    private static final float WEAPON_DAMAGE = 4.0f; // Buffé : Dégâts par balle modérés, mais compensés par la rafale
    private static final float PAP_BONUS_DAMAGE = 5.0f; // Buffé : Bon bonus Pack-a-Punch
    
    private static final float BASE_HEADSHOT_DAMAGE = 1.8f; // Multiplicateur headshot décent
    private static final float PAP_HEADSHOT_BONUS = 3.0f; // Buffé : Bonus de dégâts PaP headshot
    
    private static final float BASE_PROJECTILE_VELOCITY = 2.2f; // Vitesse de projectile correcte
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; 
    private static final float BASE_PROJECTILE_SPREAD = 1.2f; // Plus de dispersion qu'un pistolet précis en raison du tir en rafale
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.5f; // Buffé : Amélioration de la précision PaP

    private static final float BASE_RECOIL_PITCH = 1.2f; // Recul plus prononcé à cause de la rafale
    private static final float BASE_RECOIL_YAW = 0.6f; 
    private static final float PAP_RECOIL_MULTIPLIER = 0.75f; 

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.BERETTA93R_FIRE.get(); // À définir
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.BERETTA93R_RELOADING.get(); // À définir
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§cLe Triplé Mortel", "§cThe Deadly Triple") // Translated PaP name
            : getTranslatedMessage("§7Beretta 93R", "§7Beretta 93R"); // Beretta 93R name likely same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§ePistolet semi-automatique capable de tirer en rafale de 3 coups.", "§eSemi-automatic pistol capable of firing in 3-round bursts.")));
        if (upgraded) {
