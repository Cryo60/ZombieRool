...
public class Maschinenpistole28WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 24; // Chargeur historique, un peu petit
    private static final int MAX_RESERVE    = MAX_AMMO * 6; // Réserve correcte pour un SMG d'époque
    private static final int COOLDOWN_TICKS = 5; // Cadence de tir modérée pour un SMG ancien
    private static final int RELOAD_TIME    = 70; // Rechargement un peu lent

    private static final float WEAPON_DAMAGE = 8.0f; // Dégâts de base suffisants pour les premières vagues
    private static final float PAP_BONUS_DAMAGE = 9.0f; // Bon bonus PaP pour la maintenir viable un peu plus longtemps

    private static final float BASE_HEADSHOT_DAMAGE = 15.0f; // Multiplicateur de headshot décent
    private static final float PAP_HEADSHOT_BONUS = 7.0f;    // Bonus headshot PaP pour l'encourager

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 2.5f; // Vitesse de projectile moyenne
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Amélioration modeste de vitesse
    private static final float BASE_PROJECTILE_SPREAD = 1.0f; // Dispersion notable
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.7f; // Amélioration de la précision PaP nécessaire

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 1.0f; // Recul vertical perceptible
    private static final float BASE_RECOIL_YAW = 0.6f;   // Recul horizontal présent
    private static final float PAP_RECOIL_MULTIPLIER = 0.75f; // Réduction du recul PaP pour un meilleur contrôle


    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.MP28_FIRE.get(); // Utilise MP28_FIRE
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.MP28_RELOADING.get(); // Utilise MP28_RELOADING
    private static final SoundEvent DRY_FIRE_SOUND       = ZombieroolModSounds.RIFLE_DRY.get(); // Générique pour l'instant
    ...
    public Maschinenpistole28WeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.COMMON)); // Rareté COMMON, cohérent avec son rôle
    }
    ...
    @Override public void setReserve(ItemStack s, int r) { getOrCreateTag(s).putInt("Reserve", Mth.clamp(r, 0, MAX_RESERVE)); } // Clamped reserve
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§5La Mitrailleuse Rustique", "§5The Rustic Machine Gun") // Translated PaP name
            : getTranslatedMessage("§2Maschinenpistole 28", "§2Maschinenpistole 28"); // Base name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§ePistolet-mitrailleur ancien", "§eOld Submachine Gun")));
        tips.add(Component.literal(getTranslatedMessage("§fCadence de tir modérée, efficace à courte portée.", "§fModerate fire rate, effective at short range.")));
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : La Mitrailleuse Rustique", "§dPaP Name: The Rustic Machine Gun")));
            ...
            tips.add(Component.literal(getTranslatedMessage("§7Un classique fiable des premières vagues.", "§7A reliable classic for early waves.")));
        }
