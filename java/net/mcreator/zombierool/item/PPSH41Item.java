...
public class PPSH41Item extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO = 50; // Chargeur tambour emblématique de 71 balles
    private static final int MAX_RESERVE = 284; // 4 chargeurs complets en réserve
    private static final int COOLDOWN_TICKS = 1; // Cadence de tir EXTRÊME (identique à la Thompson)
    private static final int RELOAD_TIME = 70; // Plus long à recharger, 3.5 secondes
    
    private static final float WEAPON_DAMAGE = 7.0f; // Dégâts de base légèrement supérieurs à la Thompson
    private static final float PAP_BONUS_DAMAGE = 13.0f; // Très bon bonus PaP, pour la maintenir viable
    
    private static final float BASE_HEADSHOT_DAMAGE = 2.3f; // Bon multiplicateur headshot
    private static final float PAP_HEADSHOT_BONUS = 0.9f; // Bonus headshot si PaP
    
    // Paramètres de tir (projectile invisible)
    private static final float BASE_PROJECTILE_VELOCITY = 2.9f; // Vitesse des balles
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.18f; 
    private static final float BASE_PROJECTILE_SPREAD = 1.4f; // Dispersion élevée à la base
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.55f; // Bonne réduction de dispersion avec PaP

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 1.3f; // Recul vertical élevé
    private static final float BASE_RECOIL_YAW = 0.25f; // Recul latéral marqué
    private static final float PAP_RECOIL_MULTIPLIER = 0.65f; // Recul réduit avec PaP

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.PPSH41_FIRE.get(); // À définir dans ZombieroolModSounds
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.PPSH41_RELOADING.get(); // À définir
    ...
    public PPSH41Item() {
        super(new Properties().stacksTo(1).rarity(Rarity.RARE)); // Rareté rare pour cette arme iconique
    }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§5Démone à Tambour", "§5Drum Demon") // Translated PaP name
            : getTranslatedMessage("§fPPSh-41", "§fPPSh-41"); // PPSh-41 name likely same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        ...
        tips.add(Component.literal(getTranslatedMessage("§aCadence de tir infernale et chargeur massif pour le contrôle de foule.", "§aInfernal rate of fire and massive magazine for crowd control.")));
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : Démone à Tambour", "§dPaP Name: Drum Demon"))); // Specific PaP name - now translated
