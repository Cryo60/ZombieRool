...
public class RPGWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    // --- Statistiques de l'arme (9/10) ---
    private static final int MAX_AMMO       = 1; // Un seul tir par chargeur
    private static final int MAX_RESERVE    = MAX_AMMO * 3; // Très peu de roquettes en réserve
    private static final int COOLDOWN_TICKS = 40; // Temps entre les tirs (très peu pertinent pour 1 balle)
    private static final int RELOAD_TIME    = 80; // Long temps de rechargement

    private static final float WEAPON_DAMAGE = 20.0f; // Dégâts directs élevés, même si l'AoE est la clé
    private static final float PAP_BONUS_DAMAGE = 30.0f; // Très gros bonus pour le Pack-a-Punch

    private static final float BASE_HEADSHOT_DAMAGE = 1.0f; // Headshots moins pertinents sur un explosif
    private static final float PAP_HEADSHOT_BONUS = 2.0f;    // Bonus headshot faible même si PaP

    // Projectile Parameters (lent, gros et puissant)
    private static final float BASE_PROJECTILE_VELOCITY = 1.0f; // Projectile très lent, facile à esquiver pour le joueur
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Légèrement plus rapide
    private static final float BASE_PROJECTILE_SPREAD = 0.5f; // Précision relative pour un lance-roquette
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 1.0f; // Pas de changement, c'est déjà précis pour ce type

    // Recoil Parameters (très fort recul pour un lance-roquette)
    private static final float BASE_RECOIL_PITCH = 4.0f; // Recul vertical colossal
    private static final float BASE_RECOIL_YAW = 2.0f;   // Recul horizontal important
    private static final float PAP_RECOIL_MULTIPLIER = 0.7f; // Recul un peu réduit avec PaP

    // --- Explosion Parameters ---
    public static final String TAG_IS_RPG_PROJECTILE = "zombierool:is_rpg_projectile"; // Nouveau tag pour l'ArrowImpactHandler
    public static final float BASE_EXPLOSION_RADIUS = 5.0f; // Très large rayon d'explosion
    public static final float PAP_EXPLOSION_RADIUS_MULTIPLIER = 1.3f; // Rayon encore plus grand avec PaP
    public static final float BASE_EXPLOSION_DAMAGE_MULTIPLIER = 2.0f; // Multiplicateur de dégâts de l'explosion
    public static final float PAP_EXPLOSION_DAMAGE_MULTIPLIER = 4.0f; // Très très gros multiplicateur pour l'explosion PaP

    // --- Sons de l'arme ---
    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.RPG_FIRE.get(); // À adapter si tu as un son spécifique
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Son générique
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.RPG_RELOADING.get(); // À adapter
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        if (getAmmo(stack) > 0) {
            ...
            // Marquer cette flèche comme un projectile de l'RPG pour l'ArrowImpactHandler
            arrow.getPersistentData().putBoolean(TAG_IS_RPG_PROJECTILE, true);
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§5Destructeur d'Hordes", "§5Horde Destroyer") // Translated PaP name
            : getTranslatedMessage("§4RPG-7", "§4RPG-7"); // Base name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§cLance-roquettes puissant, idéal pour les gros groupes de zombies.", "§cPowerful rocket launcher, ideal for large groups of zombies."))); // Description
        if (upgraded) {
            ...
            tips.add(Component.literal(getTranslatedMessage(
                "§5Bonus dégâts AoE PaP : " + String.format("%.1f", getExplosionDamageMultiplier(stack)) + "x",
                "§5PaP AoE Damage Bonus: " + String.format("%.1f", getExplosionDamageMultiplier(stack)) + "x"
            )));
        }
    }
