...
public class Starr1858WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 6; // Petit barillet
    private static final int MAX_RESERVE    = MAX_AMMO * 3; // Très peu de munitions en réserve
    private static final int COOLDOWN_TICKS = 15; // Tir très lent, coup par coup, pistolet à poudre
    private static final int RELOAD_TIME    = 80; // Rechargement extrêmement lent (pour simuler un rechargement manuel)

    private static final float WEAPON_DAMAGE = 0.7f; // Dégâts faibles pour compenser la lenteur
    private static final float PAP_BONUS_DAMAGE = 5.0f; // Très faible bonus PaP, l'arme reste mauvaise

    private static final float BASE_HEADSHOT_DAMAGE = 10.0f; // Dégâts de headshot de base peu impactants
    private static final float PAP_HEADSHOT_BONUS = 3.0f;    // Bonus headshot PaP quasi inexistant

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 2.0f; // Projectile lent
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.0f; // Pas d'amélioration PaP significative sur la vitesse
    private static final float BASE_PROJECTILE_SPREAD = 1.5f; // Très large dispersion
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 1.0f; // Pas d'amélioration de la précision PaP

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 1.5f; // Très fort recul vertical
    private static final float BASE_RECOIL_YAW = 0.8f;   // Fort recul horizontal
    private static final float PAP_RECOIL_MULTIPLIER = 1.0f; // Pas de réduction de recul PaP


    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.S1858_FIRE.get(); // Utilise S1858 comme tu l'as défini
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.S1858_RELOADING.get(); // Utilise S1858_RELOADING
    private static final SoundEvent DRY_FIRE_SOUND       = ZombieroolModSounds.RIFLE_DRY.get(); // Utilise un son de tir à vide générique pour l'instant
    ...
    public Starr1858WeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.COMMON)); // Rareté COMMON, car c'est une arme de départ
    }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§fLe Vieux Pataud", "§fThe Old Clunker") // Translated PaP name
            : getTranslatedMessage("§7Starr-1858", "§7Starr-1858"); // Starr-1858 name (remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§ePistolet de départ (très basique)", "§eStarting Pistol (very basic)")));
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch (ne vous attendez pas à des miracles)", "§dUpgraded via Pack-a-Punch (don't expect miracles)")));
            tips.add(Component.literal(getTranslatedMessage(
                "§dBonus dégâts PaP : " + String.format("%.1f", PAP_BONUS_DAMAGE), // Use PAP_BONUS_DAMAGE for consistency
                "§dPaP Damage Bonus: " + String.format("%.1f", PAP_BONUS_DAMAGE)
            )));
            // Added for consistency with other weapons, even if multiplier is 1.0f
            tips.add(Component.literal(getTranslatedMessage("§dPas d'amélioration significative de la précision ou du recul.", "§dNo significant improvement to accuracy or recoil.")));
        }
    }
