...
public class OldCrossbowWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 1; // Crossbow has 1 round in the magazine (single shot)
    private static final int MAX_RESERVE    = 30; // 30 bolts in reserve
    private static final int COOLDOWN_TICKS = 0; // No real cooldown after shooting, but reload is very long
    private static final int RELOAD_TIME    = 60; // ticks (3s for crossbow reload, quite long)
       
    private static final float WEAPON_DAMAGE = 12.0f; // Decent damage for 5/10 (can one-shot early/mid-game zombies)
    private static final float PAP_BONUS_DAMAGE = 20.0f; // Significant Pack-a-Punch damage bonus for late game
       
    private static final float BASE_HEADSHOT_DAMAGE = 2.0f; // Good headshot multiplier
    private static final float PAP_HEADSHOT_BONUS = 3.0f;    // Bonus headshot damage if PaP
    
    // Bullet Parameters
    private static final float BASE_PROJECTILE_VELOCITY = 3.5f; // Faster projectile speed
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Slight speed multiplier for PaP
    private static final float BASE_PROJECTILE_SPREAD = 0.0f; // Very accurate, almost no spread for crossbow
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 1.0f; // No change for PaP, already perfect

    // Recoil Parameters
    private static final float BASE_RECOIL_PITCH = 0.0f; // Negligible recoil for crossbow
    private static final float BASE_RECOIL_YAW = 0.0f;   // Negligible recoil for crossbow
    private static final float PAP_RECOIL_MULTIPLIER = 1.0f; // No change for PaP, already no recoil


    private static final SoundEvent FIRE_SOUND           = SoundEvents.CROSSBOW_SHOOT;
    private static final SoundEvent FIRE_SOUND_UPGRADED = SoundEvents.CROSSBOW_QUICK_CHARGE_3; // Placeholder for upgraded sound
    private static final SoundEvent RELOAD_SOUND         = SoundEvents.CROSSBOW_LOADING_END; // Sound at the end of reloading
    // CHANGED THIS LINE: Used STONE_BUTTON_CLICK_OFF as a generic click sound
    private static final SoundEvent DRY_FIRE_SOUND       = SoundEvents.STONE_BUTTON_CLICK_OFF;
    private static final SoundEvent WEAPON_IN_HAND_SOUND = SoundEvents.ITEM_PICKUP; // Generic item pickup sound
    ...
    // Adapted for Crossbow: small explosive effect on shot rather than reload
    public static class CrossbowCherryColaEffects {
        public static final double RADIUS = 2.0;
        public static final float PERCENTAGE_DAMAGE = 0.05f; // 5% of max health
        public static final float FLAT_DAMAGE_BONUS = 5.0f; // Flat damage for close enemies
        public static final int STUN_DURATION_TICKS = 40;
    }
    // --- End of internal class ---

    public OldCrossbowWeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.UNCOMMON)); // StacksTo 1 for weapons
    }
    ...
    public void startReload(ItemStack stack, Player player) {
        ...
        if (getReloadTimer(stack) == 0 && getAmmo(stack) < MAX_AMMO && getReserve(stack) > 0) {
            ...
            if (!player.level().isClientSide) { // This block runs on the server
                ...
                player.level().playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.CROSSBOW_LOADING_START, SoundSource.PLAYERS, 1f, pitch);    
    ...
    private void finishReload(ItemStack stack, Player player, Level level) {
        ...
        level.playSound(null, player.getX(), player.getY(), player.getZ(), RELOAD_SOUND, SoundSource.PLAYERS, 1f, 1f); // Play final reload sound
        sp.connection.send(new ClientboundSetEquipmentPacket(
            sp.getId(), List.of(Pair.of(EquipmentSlot.MAINHAND, stack))));
    }
    ...
    @Override public UseAnim getUseAnimation(ItemStack s) { return getReloadTimer(s) > 0 ? UseAnim.NONE : UseAnim.BOW; } // Use BOW for crossbow animation
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        if (currentTick - lastFireTick < COOLDOWN_TICKS) { // COOLDOWN_TICKS is 0, so this won't block immediate shot
            return;
        }
        ...
        if (getAmmo(stack) > 0) {
            ...
            arrow.getPersistentData().putBoolean("zombierool:small", true); // Keep small to distinguish from vanilla arrows
            ...
            // Cherry Cola effect on shot for Crossbow
            if (player.hasEffect(ZombieroolModMobEffects.PERKS_EFFECT_CHERRY.get())) {
                double radius = CrossbowCherryColaEffects.RADIUS;
                float percentageDamage = CrossbowCherryColaEffects.PERCENTAGE_DAMAGE;
                float flatDamageBonus = CrossbowCherryColaEffects.FLAT_DAMAGE_BONUS;
                int stunDurationTicks = CrossbowCherryColaEffects.STUN_DURATION_TICKS;
                ...
                for (LivingEntity target : entities) {   
                    ...
                    if (target instanceof Monster || (target instanceof TamableAnimal && !((TamableAnimal)target).isTame()) || (target instanceof Animal && !(target instanceof TamableAnimal))) {   
                        ...
                        damage += flatDamageBonus; // Add flat bonus as it's a "thump" effect
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity ent, int slot, boolean sel) {
        ...
        if (level.isClientSide) {
            ...
            // No Cherry animation on reload for crossbow, effect is on shot
            return;
        }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§dLe Coup de Tonnerre", "§dThe Thunderclap")
            : getTranslatedMessage("§2Vieille Arbalète", "§2Old Crossbow");
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eArme à tir unique, puissante mais lente.", "§eSingle-shot weapon, powerful but slow.")));
        if (upgraded) {
