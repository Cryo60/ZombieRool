...
public class TroicinqseptmagnumItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon, IHandgunWeapon {

    private static final int MAX_AMMO = 6; // Un revolver, 6 coups
    private static final int MAX_RESERVE = MAX_AMMO * 10; // Bonne réserve pour un pistolet
    private static final int COOLDOWN_TICKS = 15; // Un peu lent, c'est un magnum
    private static final int RELOAD_TIME = 120; // ticks (2.5s) pour un rechargement pièce par pièce

    // Le .357 est puissant de base
    private static final float WEAPON_DAMAGE = 13.0f; 
    private static final float PAP_BONUS_DAMAGE = 15.0f; // Bon bonus pour le rendre viable plus longtemps

    // Headshot, important pour un pistolet de précision
    private static final float BASE_HEADSHOT_DAMAGE = 2.0f; // Multiplicateur (ex: 18 * 2 = 36)
    private static final float PAP_HEADSHOT_BONUS = 1.5f; // Multiplicateur supplémentaire PaP (ex: 36 * 1.5 = 54)

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 4.0f; // Rapide et direct
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Un peu plus rapide
    private static final float BASE_PROJECTILE_SPREAD = 0.05f; // Très précis de base
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.8f; // Encore plus précis

    // Paramètres de Recul - Un magnum, ça recule !
    private static final float BASE_RECOIL_PITCH = 1.5f;
    private static final float BASE_RECOIL_YAW = 0.8f;
    private static final float PAP_RECOIL_MULTIPLIER = 0.5f; // Le PaP réduit bien le recul

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.TROICINQSEPT_FIRE.get();
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Tu peux créer un son spécifique si tu veux
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.TROICINQSEPT_RELOADING.get();
    private static final SoundEvent DRY_FIRE_SOUND = ZombieroolModSounds.RIFLE_DRY.get(); // Tu peux créer un son spécifique pour le clic à vide
    ...
    public TroicinqseptmagnumItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.UNCOMMON)); // Un revolver est plus rare qu'une arme de base
    }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§6Le Dernier Rempart", "§6The Last Stand") // Translated PaP name
            : getTranslatedMessage("§7.357 Magnum", "§7.357 Magnum"); // Magnum name likely same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eRevolver puissant et précis", "§ePowerful and precise revolver")));
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            // Affichage du bonus de dégâts PaP (WEAPON_DAMAGE + PAP_BONUS_DAMAGE)
            ...
            // Affichage du multiplicateur de headshot PaP (BASE_HEADSHOT_DAMAGE * PAP_HEADSHOT_BONUS)
            tips.add(Component.literal(getTranslatedMessage(
