...
import net.minecraft.client.Minecraft; // AJOUTÉ : Import pour la vérification de la langue client

public class NeedlerWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 26; // Needler's magazine size
    private static final int MAX_RESERVE    = MAX_AMMO * 7; // Total reserve ammo
    private static final int COOLDOWN_TICKS = 2; // Rapid fire
    private static final int RELOAD_TIME    = 35; // ticks (1.75 seconds)

    private static final float WEAPON_DAMAGE = 2.0f;
    private static final float PAP_BONUS_DAMAGE = 2.0f;

    private static final float BASE_HEADSHOT_DAMAGE = 0.0f;
    private static final float PAP_HEADSHOT_BONUS = 0.0f;

    private static final float BASE_PROJECTILE_VELOCITY = 0.8f;
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.2f;
    private static final float BASE_PROJECTILE_SPREAD = 1.0f;
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.7f;

    private static final float BASE_RECOIL_PITCH = 0.1f;
    private static final float BASE_RECOIL_YAW = 0.05f;
    private static final float PAP_RECOIL_MULTIPLIER = 0.3f;

    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.NEEDLER_FIRE.get();
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.NEEDLER_RELOADING.get();
    ...
    private static final String TAG_NEEDLER_HOMING = "zombierool:needler_homing"; // **NEW TAG FOR HOMING**
    ...
    private static boolean isEnglishClient() {
        // Cette méthode doit être appelée côté client, mais en théorie appendHoverText
        // et getName le sont. On doit gérer le cas où Minecraft n'est pas encore prêt.
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        if (getAmmo(stack) > 0) {
            ...
            arrow.getPersistentData().putBoolean(TAG_NEEDLER_PROJECTILE, true);
            arrow.getPersistentData().putBoolean(TAG_NEEDLER_HOMING, true); // **SET HOMING TAG**
    ...
    public Component getName(ItemStack stack) {
        ...
        String frenchName = upgraded ? "§dTempête de Cristal" : "§2Needler";
        String englishName = upgraded ? "§dCrystal Storm" : "§2Needler"; // Needler reste Needler
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        ...
        String desc1Fr = "§eLance-aiguilles Covenant à tête chercheuse";
        String desc1En = "§eCovenant homing needle launcher";
        tips.add(Component.literal(getTranslatedMessage(desc1Fr, desc1En)));
        
        String desc2Fr = "§eLes aiguilles s'attachent et provoquent une §cSupercombine§e explosive !";
        String desc2En = "§eNeedles attach and cause an explosive §cSupercombine§e!";
        tips.add(Component.literal(getTranslatedMessage(desc2Fr, desc2En)));

        if (upgraded) {
            String papDesc1Fr = "§dAméliorée via Pack-a-Punch";
            String papDesc1En = "§dUpgraded via Pack-a-Punch";
            tips.add(Component.literal(getTranslatedMessage(papDesc1Fr, papDesc1En)));

            String papDesc2Fr = "§dAugmente les dégâts par aiguille et la vitesse du projectile.";
            String papDesc2En = "§dIncreases damage per needle and projectile speed.";
            tips.add(Component.literal(getTranslatedMessage(papDesc2Fr, papDesc2En)));
        }
    }
