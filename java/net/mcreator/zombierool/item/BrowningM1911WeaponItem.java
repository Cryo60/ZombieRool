...
public class BrowningM1911WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO = 100; // Grande capacité de chargeur
    private static final int MAX_RESERVE = MAX_AMMO * 5; // Grande réserve
    private static final int COOLDOWN_TICKS = 4; // Cadence de tir solide (0.2s entre les tirs)
    private static final int RELOAD_TIME = 90; // Rechargement assez lent (4.5s)
    
    private static final float WEAPON_DAMAGE = 9.0f; // Dégâts de base élevés (tue en 1 coup les zombies de base)
    private static final float PAP_BONUS_DAMAGE = 12.0f; // Bon bonus Pack-a-Punch
    
    private static final float BASE_HEADSHOT_DAMAGE = 2.2f; // Multiplicateur headshot élevé
    private static final float PAP_HEADSHOT_BONUS = 8.0f; 
    // Dégâts de base du headshot : 9.0 * 2.2 = 19.8
    // Dégâts PaP du headshot : (9.0 + 12.0) * 2.2 + 8.0 = 21.0 * 2.2 + 8.0 = 46.2 + 8.0 = 54.2
    ...
    private static final float BASE_PROJECTILE_SPREAD = 1.2f; // Précision modérée
    ...
    private static final float BASE_RECOIL_YAW = 0.7f; // Recul horizontal modéré
    private static final float PAP_RECOIL_MULTIPLIER = 0.6f; // Bonne amélioration du recul PaP

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.M1911_LMG_FIRE.get(); // À définir
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.M1911_LMG_RELOADING.get(); // À définir
    ...
    public BrowningM1911WeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.RARE)); // Rareté Rare pour une LMG
    }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§eCalamité", "§eCalamity") // Translated PaP name
            : getTranslatedMessage("§6Browning", "§6Browning"); // Base name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eOffre une capacité de munitions élevée et une bonne puissance de feu.", "§eOffers high ammo capacity and good firepower.")));
        if (upgraded) {
