...
public class InterventionWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO = 7; // Capacité du chargeur, petite mais puissante
    private static final int MAX_RESERVE = MAX_AMMO * 8; // Bonne réserve pour tenir les vagues
    private static final int COOLDOWN_TICKS = 20; // Sniper: 1 seconde par tir, pour une bonne cadence
    private static final int RELOAD_TIME = 80; // ticks (4s), un peu long pour un sniper

    private static final float WEAPON_DAMAGE = 25.0f; // Dégâts de base très élevés
    private static final float PAP_BONUS_DAMAGE = 35.0f; // Énorme bonus PaP pour la fin de partie

    private static final float BASE_HEADSHOT_DAMAGE = 75.0f; // OS les zombies pendant très longtemps
    private static final float PAP_HEADSHOT_BONUS = 50.0f; // Bonus headshot PaP pour les vagues extrêmes

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 4.5f; // Projectile très rapide
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Vitesse légèrement améliorée PaP
    private static final float BASE_PROJECTILE_SPREAD = 0.05f; // Très précis
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.1f; // Quasi parfait après PaP (0.005)

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 1.2f; // Recul vertical notable
    private static final float BASE_RECOIL_YAW = 0.1f; // Très peu de recul horizontal
    private static final float PAP_RECOIL_MULTIPLIER = 0.2f; // Recul quasi inexistant après PaP

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.INTERVENTION_FIRE.get(); // Créer un son unique
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.INTERVENTION_FIRE.get(); // Son amélioré
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.INTERVENTION_RELOADING.get(); // Son de rechargement unique
    private static final SoundEvent DRY_FIRE_SOUND = ZombieroolModSounds.RIFLE_DRY.get(); // Son "clic" si plus de balles
    private static final SoundEvent WEAPON_IN_HAND_SOUND = ZombieroolModSounds.WEAPON_IN_HAND.get(); // Son de prise en main
    private static final SoundEvent BOLT_SOUND = ZombieroolModSounds.INTERVENTION_BOLT.get(); // Son du réarmement du verrou
    ...
    public InterventionWeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.EPIC)); // Rareté Épique pour un sniper 10/10
    }
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        if (getAmmo(stack) > 0) {
            ...
            // Play the bolt sound
            level.playSound(null, player.getX(), player.getY(), player.getZ(), BOLT_SOUND, SoundSource.PLAYERS, 0.7f, 1f);
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§eNostalgie", "§eNostalgia") // Translated PaP name
            : getTranslatedMessage("§6Intervention", "§6Intervention"); // Base name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§cFusil de précision Bolt-Action", "§cBolt-Action Sniper Rifle")));
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§eAméliorée via Pack-a-Punch", "§eUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage(
                "§eBonus dégâts PaP : " + String.format("%.1f", getHeadshotPapBonusDamage(stack)),
                "§ePaP Damage Bonus: " + String.format("%.1f", getHeadshotPapBonusDamage(stack))
            )));
            tips.add(Component.literal(getTranslatedMessage("§cCapable de One-Shot les zombies pendant de très nombreuses vagues en headshot.", "§cCapable of One-Shotting zombies with headshots for many waves."))); // Translated descriptive text
        }
    }
