...
public class MP5WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO = 50; // Buffé : Plus grand chargeur qu'une M16A4 pour une SMG
    private static final int MAX_RESERVE = MAX_AMMO * 6; // Buffé : Réserve légèrement plus petite proportionnellement
    private static final int COOLDOWN_TICKS = 2; // Buffé : Tir rapide pour une SMG (0.10 secondes)
    private static final int RELOAD_TIME = 50; // Recharge correcte (2.5s)
           
    private static final float WEAPON_DAMAGE = 6.5f; // Buffé : Dégâts de base légèrement augmentés
    private static final float PAP_BONUS_DAMAGE = 8.0f; // Buffé : Bon bonus PaP pour la rendre viable
           
    private static final float BASE_HEADSHOT_DAMAGE = 10.0f; // Buffé : Multiplicateur de headshot décent
    private static final float PAP_HEADSHOT_BONUS = 7.0f; // Buffé : Bonus de dégâts du headshot si PaP
    
    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 2.8f; // Vitesse de projectile standard
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Léger boost PaP
    private static final float BASE_PROJECTILE_SPREAD = 1.0f; // Buffé : Dispersion un peu plus élevée qu'un fusil d'assaut
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.5f; // Buffé : Bonne amélioration de la dispersion PaP
    
    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 0.5f; // Recul vertical modéré
    private static final float BASE_RECOIL_YAW = 0.6f;   // Recul horizontal plus prononcé pour une SMG
    private static final float PAP_RECOIL_MULTIPLIER = 0.5f; // Bon gain sur le recul PaP

    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.MP5_FIRE.get(); // Son silencieux
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Peut être un son PaP plus puissant
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.MP5_RELOADING.get();
    ...
    public MP5WeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.UNCOMMON)); // Rarité UNCOMMON car c'est une arme à part entière
    }
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        if (getAmmo(stack) > 0) {
            ...
            // Pour une MP5 silencieuse, le son de tir est faible, voire muet à la base
            level.playSound(null, 
                player.getX(), player.getY(), player.getZ(),
                getFireSound(stack),
                SoundSource.PLAYERS, 0.3f, 1f // Volume très faible pour le son silencieux
            );
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§dMP5SD", "§dMP5SD") // MP5SD name (proper noun, remains same)
            : getTranslatedMessage("§2MP5 Silencieuse", "§2MP5 Silenced"); // Translated base name
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§ePistolet-mitrailleur (silencieux)", "§eSubmachine Gun (Silenced)")));
        tips.add(Component.literal(getTranslatedMessage("§aUn pistolet-mitrailleur avec un chargeur étendu et une cadence de tir rapide.", "§aA submachine gun with an extended magazine and a high rate of fire."))); // Added descriptive text
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : MP5SD", "§dPaP Name: MP5SD"))); // Specific PaP name - now translated
            ...
            tips.add(Component.literal(getTranslatedMessage("§dDispersion et recul grandement améliorés.", "§dGreatly improved spread and recoil."))); // Added descriptive text
        }
    }
