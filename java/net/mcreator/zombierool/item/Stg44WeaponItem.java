...
public class Stg44WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 25;   // 25 balles par chargeur (Réduit)
    private static final int MAX_RESERVE    = 150;  // 150 balles en réserve (6 chargeurs, Réduit)
    private static final int COOLDOWN_TICKS = 4;    // Cadence de tir légèrement réduite (4 tirs par seconde)
    private static final int RELOAD_TIME    = 55;   // 2.75 secondes pour recharger (Légèrement augmenté)

    private static final float WEAPON_DAMAGE        = 12.0f;  // Dégâts modérés par balle (Réduit)
    private static final float PAP_BONUS_DAMAGE     = 8.0f;   // Bonus de dégâts PaP (Réduit)

    private static final float BASE_HEADSHOT_DAMAGE = 2.0f;   // Bon bonus de headshot (Réduit)
    private static final float PAP_HEADSHOT_BONUS   = 0.3f;   // Petit bonus supplémentaire au PaP (Réduit)

    // Paramètres de tir (projectile invisible)
    private static final float BASE_PROJECTILE_VELOCITY = 3.2f; // Vitesse des balles légèrement réduite
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.05f; // Moins rapide après PaP
    private static final float BASE_PROJECTILE_SPREAD = 1.0f;   // Dispersion légèrement augmentée
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.6f; // Précision avec PaP un peu moins bonne

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 1.2f; // Recul vertical légèrement augmenté
    private static final float BASE_RECOIL_YAW = 0.2f;  // Léger recul latéral légèrement augmenté
    private static final float PAP_RECOIL_MULTIPLIER = 0.8f; // Recul réduit avec PaP (moins efficace)

    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.STG44_FIRE.get(); // Créer ce son
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Créer ce son
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.STG44_RELOADING.get(); // Créer ce son
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§4Sturm-Schraube", "§4Sturm-Schraube") // Translated PaP name
            : getTranslatedMessage("§fSTG-44", "§fSTG-44"); // STG-44 name likely same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§l§eFusil d'Assaut", "§l§eAssault Rifle")));
        tips.add(Component.literal(getTranslatedMessage("§aRobuste et précis, parfait pour les premières et moyennes vagues.", "§aRobust and accurate, perfect for early and mid-waves.")));
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : Sturm-Schraube", "§dPaP Name: Sturm-Schraube"))); // Specific PaP name
