...
import net.minecraft.network.chat.MutableComponent; // Import needed for the translation system
...
import net.minecraft.client.Minecraft; // Import needed for the translation system
...
public class RPDWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    // --- Translation Helper Methods (Copied from your system) ---

    /**
     * Checks if the client's language is set to English (en_us, en_gb, etc.).
     */
    private static boolean isEnglishClient() {
        // Minecraft.getInstance() can be null on the server side, but this item
        // class is mainly used server-side for gameplay logic. However, translation
        // is only relevant client-side (tooltip, item name).
        if (Minecraft.getInstance() == null) {
            return false; // Assume non-English if we can't check (e.g., dedicated server)
        }
    ...
    // --- Weapon Stats ---
    private static final int MAX_AMMO = 100; // Très grande capacité de chargeur, typique des LMG
    private static final int MAX_RESERVE = MAX_AMMO * 6; // Très grande réserve pour le soutien prolongé
    private static final int COOLDOWN_TICKS = 4; // Cadence de tir modérée (0.2s entre les tirs)
    private static final int RELOAD_TIME = 90; // Rechargement lent (4.5s), un désavantage clé des LMG
    
    private static final float WEAPON_DAMAGE = 8.0f; // Bons dégâts de base pour un LMG
    private static final float PAP_BONUS_DAMAGE = 12.0f; // Très bon bonus Pack-a-Punch pour les vagues avancées
    
    private static final float BASE_HEADSHOT_DAMAGE = 1.8f; // Multiplicateur headshot correct
    private static final float PAP_HEADSHOT_BONUS = 7.0f; 
    ...
    private static final float BASE_PROJECTILE_SPREAD = 1.5f; // Dispersion assez prononcée en tir prolongé
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.7f; // Amélioration notable de la précision PaP

    private static final float BASE_RECOIL_PITCH = 1.0f; // Recul vertical important
    private static final float BASE_RECOIL_YAW = 0.7f; // Recul horizontal aussi présent
    private static final float PAP_RECOIL_MULTIPLIER = 0.6f; // Amélioration significative du recul PaP

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.RPD_FIRE.get(); // À définir
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.RPD_RELOADING.get(); // À définir
    ...
    public RPDWeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.UNCOMMON)); // Rareté commune à non commune pour une LMG de base
    }
    ...
    public Component getName(ItemStack stack) {
        ...
        String baseName = "§6RPD";
        String papNameFr = "§cGros Tonnerre"; // Translation for "Big Thunder"
        String papNameEn = "§cBig Thunder";
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        ...
        tips.add(getTranslatedComponent(
            "§eMitrailleuse lourde offrant une capacité de munitions élevée et des dégâts solides.",
            "§eHeavy machine gun offering high ammo capacity and solid damage."
        ));
