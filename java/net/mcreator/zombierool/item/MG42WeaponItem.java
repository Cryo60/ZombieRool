...
public class MG42WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO = 150; // Très grande capacité de chargeur
    private static final int MAX_RESERVE = MAX_AMMO * 5; // Très grande réserve
    private static final int COOLDOWN_TICKS = 2; // Cadence de tir EXTRÊMEMENT rapide (0.1s entre les tirs)
    private static final int RELOAD_TIME = 60; // Rechargement TRÈS lent (6.0s)
    
    private static final float WEAPON_DAMAGE = 8.5f; // Dégâts de base solides
    private static final float PAP_BONUS_DAMAGE = 18.0f; // Excellent bonus Pack-a-Punch pour les vagues tardives
    
    private static final float BASE_HEADSHOT_DAMAGE = 1.8f; // Multiplicateur headshot modéré (moins sur une arme à forte cadence)
    private static final float PAP_HEADSHOT_BONUS = 15.0f; 
    // Dégâts de base du headshot : 8.5 * 1.8 = 15.3
    // Dégâts PaP du headshot : (8.5 + 18.0) * 1.8 + 15.0 = 26.5 * 1.8 + 15.0 = 47.7 + 15.0 = 62.7
    ...
    private static final float BASE_PROJECTILE_SPREAD = 1.8f; // Très grande dispersion de base (c'est une mitrailleuse lourde !)
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.5f; // Énorme amélioration de la précision PaP

    private static final float BASE_RECOIL_PITCH = 1.5f; // Très fort recul vertical
    private static final float BASE_RECOIL_YAW = 1.2f; // Fort recul horizontal
    private static final float PAP_RECOIL_MULTIPLIER = 0.4f; // Amélioration massive du recul PaP (pour la rendre maniable)

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.MG42_FIRE.get(); // À définir
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.MG42_RELOADING.get(); // À définir
    ...
    public MG42WeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.EPIC)); // Rareté Épique pour la "scie circulaire"
    }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§cPanzer-42", "§cPanzer-42") // Translated PaP name
            : getTranslatedMessage("§6MG42", "§6MG42"); // Base name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eLa scie circulaire de Hitler, pour un max de baston.", "§eHitler's buzzsaw, for maximum mayhem.")));
        tips.add(Component.literal(getTranslatedMessage("§eCadence de tir et capacité de munitions extrêmes.", "§eExtreme fire rate and ammo capacity.")));
        if (upgraded) {
