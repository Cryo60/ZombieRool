...
public class UMP45WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 32; // Chargeur plus petit mais impact plus lourd
    private static final int MAX_RESERVE    = MAX_AMMO * 6; // Bonne réserve pour compenser
    private static final int COOLDOWN_TICKS = 2; // Cadence de tir plus lente que la MP5 (0.2 secondes)
    private static final int RELOAD_TIME    = 55; // Recharge légèrement plus longue (2.75s)
           
    private static final float WEAPON_DAMAGE = 6.0f; // Dégâts de base solides pour une SMG (comme la M16A4)
    private static final float PAP_BONUS_DAMAGE = 9.0f; // Très bon bonus PaP, la rendant très efficace
           
    private static final float BASE_HEADSHOT_DAMAGE = 1.8f; // Multiplicateur de dégâts pour headshot (1.8x du WEAPON_DAMAGE)
    private static final float PAP_HEADSHOT_BONUS = 8.0f;    // Bonus de dégâts du headshot si PaP
    // La UMP45, avec des balles plus lourdes, est conçue pour des headshots impactants.
    // Dégâts de base du headshot : WEAPON_DAMAGE * BASE_HEADSHOT_DAMAGE_MULTIPLIER = 6.0 * 1.8 = 10.8
    // Dégâts PaP du headshot : (WEAPON_DAMAGE + PAP_BONUS_DAMAGE) * BASE_HEADSHOT_DAMAGE_MULTIPLIER + PAP_HEADSHOT_BONUS = (6.0 + 9.0) * 1.8 + 8.0 = 15.0 * 1.8 + 8.0 = 27.0 + 8.0 = 35.0

    // Paramètres de tir
    private static final float BASE_PROJECTILE_VELOCITY = 3.0f; // Vitesse de projectile standard
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.15f; // Bon boost PaP
    private static final float BASE_PROJECTILE_SPREAD = 1.0f; // Dispersion moyenne pour une SMG, mais contrôlable
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.5f; // Excellente amélioration de la dispersion PaP
    
    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 0.6f; // Recul vertical un peu plus prononcé
    private static final float BASE_RECOIL_YAW = 0.5f;   // Recul horizontal modéré
    private static final float PAP_RECOIL_MULTIPLIER = 0.5f; // Bon gain sur le recul PaP

    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.UMP45_FIRE.get(); // Placeholders pour les sons
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get();
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.UMP45_RELOADING.get();
    ...
    // --- Classe interne pour Cherry Cola Effects (Identique au modèle) ---
    ...
    // Pour l'UMP45, on va utiliser un multiplicateur pour les dégâts de tête de base.
    // Le bonus PaP sera ajouté APRES la multiplication.
    @Override
    public float getHeadshotBaseDamage(ItemStack stack) {
        // Retourne les dégâts de l'arme multipliés par le facteur BASE_HEADSHOT_DAMAGE
        return getWeaponDamage(stack) * BASE_HEADSHOT_DAMAGE;
    }

    @Override
    public float getHeadshotPapBonusDamage(ItemStack stack) {
        // Ce bonus s'ajoute aux dégâts de tête après la multiplication
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§dThe Ripper", "§dThe Ripper") // Proper noun, remains same
            : getTranslatedMessage("§2UMP45", "§2UMP45"); // Proper noun, remains same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§ePistolet-mitrailleur puissant", "§ePowerful Submachine Gun")));
        tips.add(Component.literal(getTranslatedMessage("§aUn pistolet-mitrailleur avec un fort impact et une précision décente.", "§aA submachine gun with heavy impact and decent accuracy."))); // Added descriptive text
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : The Ripper", "§dPaP Name: The Ripper"))); // Specific PaP name - now translated
            tips.add(Component.literal(getTranslatedMessage(
                "§dBonus dégâts PaP : " + String.format("%.1f", PAP_BONUS_DAMAGE), // Changed to PAP_BONUS_DAMAGE for consistency
                "§dPaP Damage Bonus: " + String.format("%.1f", PAP_BONUS_DAMAGE)
            )));
            tips.add(Component.literal(getTranslatedMessage("§dDégâts de tête et recul grandement améliorés.", "§dGreatly improved headshot damage and recoil."))); // Added descriptive text
        }
    }
