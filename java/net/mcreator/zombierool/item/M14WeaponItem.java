...
import net.minecraft.client.Minecraft;
 ...
 public class M14WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

     private static final int MAX_AMMO       = 8; // Small magazine for balance
     private static final int MAX_RESERVE    = MAX_AMMO * 8; // Good reserve for a starting weapon
     private static final int COOLDOWN_TICKS = 6; // Slower fire rate, as expected for a semi-auto starting rifle
     private static final int RELOAD_TIME    = 40; // Relatively fast reload

     private static final float WEAPON_DAMAGE = 6.0f; // Low damage for 4/10 balance, meant for early waves
     private static final float PAP_BONUS_DAMAGE = 8.0f; // Significant boost when Pack-a-Punched

     private static final float BASE_HEADSHOT_DAMAGE = 8.0f; // Good headshot multiplier for early waves
     private static final float PAP_HEADSHOT_BONUS = 8.0f;

     // Paramètres de tir
     private static final float BASE_PROJECTILE_VELOCITY = 2.8f;
     private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.3f; // Faster projectile when PaP
     private static final float BASE_PROJECTILE_SPREAD = 0.3f; // Very accurate
     private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.8f; // Even more accurate when PaP

     // Paramètres de Recul
     private static final float BASE_RECOIL_PITCH = 1.0f; // Noticeable recoil
     private static final float BASE_RECOIL_YAW = 0.5f;
     private static final float PAP_RECOIL_MULTIPLIER = 0.5f; // Much reduced recoil when PaP


     private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.M14_FIRE.get(); // Placeholder, ideally specific M14 sound
     private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Placeholder
     private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.M14_RELOADING.get(); // Placeholder
     private static final SoundEvent DRY_FIRE_SOUND       = ZombieroolModSounds.RIFLE_DRY.get(); // Placeholder
     private static final SoundEvent WEAPON_IN_HAND_SOUND = ZombieroolModSounds.WEAPON_IN_HAND.get(); // Placeholder
     ...
     public M14WeaponItem() {
         super(new Properties().stacksTo(1).rarity(Rarity.COMMON)); // Common rarity as a starting weapon
     }
    ...
    private static String getTranslatedMessage(String frenchMessage, String englishMessage) {
     ...
     public Component getName(ItemStack stack) {
         ...
         String name = upgraded
            ? getTranslatedMessage("§dLe Mitrailleur Amélioré", "§dThe Enhanced Marksman") // Translated PaP name
            : getTranslatedMessage("§2M14", "§2M14"); // M14 name (proper noun, remains same)
     ...
     public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
         initializeIfNeeded(stack);
         boolean upgraded = isPackAPunched(stack);
         tips.add(Component.literal(getTranslatedMessage("§eFusil de départ semi-automatique", "§eSemi-automatic starting rifle")));
         tips.add(Component.literal(getTranslatedMessage("§aUn fusil fiable pour les premières vagues, avec une bonne précision.", "§aA reliable rifle for early waves, with good accuracy."))); // Added descriptive text
         if (upgraded) {
             tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
             tips.add(Component.literal(getTranslatedMessage("§dNom PaP : Le Mitrailleur Amélioré", "§dPaP Name: The Enhanced Marksman"))); // Specific PaP name - now translated
