...
public class UziWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon { // Removed IHandgunWeapon as it was not defined in the provided code

    private static final int MAX_AMMO = 32; // Chargeur standard, mais vide vite avec la cadence
    private static final int MAX_RESERVE = MAX_AMMO * 5; // Bonne réserve pour soutenir le tir
    private static final int COOLDOWN_TICKS = 3; // Cadence de tir rapide (0.15s entre les tirs)
    private static final int RELOAD_TIME = 45; // Rechargement standard (2.25s)
    
    private static final float WEAPON_DAMAGE = 3.5f; // Dégâts de base faibles, son principal inconvénient
    private static final float PAP_BONUS_DAMAGE = 6.5f; // Bon bonus Pack-a-Punch pour la rendre viable
    
    private static final float BASE_HEADSHOT_DAMAGE = 1.7f; // Multiplicateur headshot décent pour compenser les faibles dégâts de base
    private static final float PAP_HEADSHOT_BONUS = 5.0f; 
    // Dégâts de base du headshot : 3.5 * 1.7 = 5.95
    // Dégâts PaP du headshot : (3.5 + 6.5) * 1.7 + 5.0 = 10.0 * 1.7 + 5.0 = 17.0 + 5.0 = 22.0
    ...
    private static final float BASE_PROJECTILE_SPREAD = 1.0f; // Dispersion assez marquée en tir automatique
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.7f; // Amélioration de la précision PaP

    private static final float BASE_RECOIL_PITCH = 0.7f; // Recul vertical notable
    private static final float BASE_RECOIL_YAW = 0.5f; // Recul horizontal aussi présent
    private static final float PAP_RECOIL_MULTIPLIER = 0.6f; // Amélioration significative du recul PaP

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.UZI_FIRE.get(); // À définir
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.UZI_RELOADING.get(); // À définir
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§cMini Chopper", "§cMini Chopper") // PaP name, remains same as it's a proper noun-like name
            : getTranslatedMessage("§7Uzi", "§7Uzi"); // Base name, remains same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§ePistolet-mitrailleur compact à cadence rapide.", "§eCompact, high-rate-of-fire submachine gun.")));
        if (upgraded) {
