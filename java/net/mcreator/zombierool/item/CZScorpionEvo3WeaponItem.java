...
import net.mcreator.zombierool.init.ZombieroolModSounds; // Ensure these sounds exist!
...
public class CZScorpionEvo3WeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    // --- Ammo & Reloading Parameters ---
    private static final int MAX_AMMO = 32; // Standard capacity for an SMG
    private static final int MAX_RESERVE = MAX_AMMO * 6; // Decent reserve
    private static final int COOLDOWN_TICKS = 3; // Very high fire rate (20 ticks per second / 3 ticks = ~6.6 shots/sec)
    private static final int RELOAD_TIME = 40; // ticks (2s), fast reload for an SMG

    // --- Damage Parameters ---
    private static final float WEAPON_DAMAGE = 8.0f; // Lower base damage per shot
    private static final float PAP_BONUS_DAMAGE = 6.0f; // Good boost with PaP

    private static final float BASE_HEADSHOT_DAMAGE = 1.8f; // Standard headshot multiplier
    private static final float PAP_HEADSHOT_BONUS = 0.5f; // Small additional bonus for PaP headshots

    // --- Firing Parameters ---
    private static final float BASE_PROJECTILE_VELOCITY = 2.0f; // Moderate projectile speed
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.1f; // Slightly faster PaP
    private static final float BASE_PROJECTILE_SPREAD = 0.6f; // Noticeable spread for SMG
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.5f; // Reduced spread with PaP

    // --- Recoil Parameters ---
    private static final float BASE_RECOIL_PITCH = 0.4f; // Moderate vertical recoil
    private static final float BASE_RECOIL_YAW = 0.2f; // Some horizontal recoil
    private static final float PAP_RECOIL_MULTIPLIER = 0.4f; // Reduced recoil with PaP

    // --- Sounds ---
    private static final SoundEvent FIRE_SOUND          = ZombieroolModSounds.SCORPION_EVO_FIRE.get(); 
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Or reuse base fire sound
    private static final SoundEvent RELOAD_SOUND        = ZombieroolModSounds.SCORPION_EVO_RELOADING.get(); 
    ...
    public CZScorpionEvo3WeaponItem() {
        super(new Item.Properties().stacksTo(1).rarity(Rarity.UNCOMMON)); // Uncommon rarity for an SMG
    }
    ...
    public void applyPackAPunch(ItemStack stack) {
        ...
        setAmmo(stack, MAX_AMMO); // Reloads ammo on PaP
        setReserve(stack, getMaxReserve()); // Refills reserve on PaP
    }
    ...
    private void finishReload(ItemStack stack, Player player, Level level) {
        ...
        int reserve = getReserve(stack) + currentAmmo; // Recover remaining bullets in mag
    ...
    @Override public UseAnim getUseAnimation(ItemStack s) { return UseAnim.BOW; } // Use BOW for continuous firing (hold click)
    @Override public int getUseDuration(ItemStack s) { return 72000; } // Allows for sustained clicking
    ...
    public void onUseTick(Level level, LivingEntity ent, ItemStack stack, int count) {
        ...
        // If out of ammo in mag but have reserve, start reload
        ...
        if (getAmmo(stack) > 0) {
            ...
            float weaponDamage = getWeaponDamage(stack); 
            ...
            arrow.setBaseDamage(weaponDamage); 
            arrow.getPersistentData().putBoolean("zombierool:smg_bullet", true); // Mark as SMG bullet
            ...
            level.playSound(null,
                ...
                getFireSound(stack),
                SoundSource.PLAYERS, 1.0f, 1f + (player.getRandom().nextFloat() * 0.1f - 0.05f) 
            );
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity ent, int slot, boolean sel) {
        ...
        if (level.isClientSide) {
            // CherryReloadAnimationHandler integration could go here if applicable
            return;
        }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§5Evo-Lution", "§5Evo-Lution") // PaP name (proper noun, remains same)
            : getTranslatedMessage("§fCZ Scorpion Evo 3", "§fCZ Scorpion Evo 3"); // Base name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        ...
        tips.add(Component.literal(getTranslatedMessage("§fCadence de tir élevée, efficace à courte et moyenne portée.", "§fHigh rate of fire, effective at short to medium range.")));
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§5Améliorée via Pack-a-Punch", "§5Upgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§5Nom PaP : Evo-Lution", "§5PaP Name: Evo-Lution"))); 
            tips.add(Component.literal(getTranslatedMessage(
                "§5Bonus dégâts : +", "§5Damage Bonus: +"
            ) + String.format("%.1f", PAP_BONUS_DAMAGE)));
            tips.add(Component.literal(getTranslatedMessage("§5Cadence de tir et précision améliorées, recul réduit.", "§5Improved fire rate and accuracy, reduced recoil.")));
        } else {
    ...
    public void tickReload(ItemStack stack, Player player, Level level) {
        // Reload logic is handled in inventoryTick, this method can remain empty.
    }
}
