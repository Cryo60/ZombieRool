...
public class ThompsonWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO       = 50;   // Chargeur tambour : 50 balles
    private static final int MAX_RESERVE    = 200;  // 200 balles en réserve (4 chargeurs tambour)
    private static final int COOLDOWN_TICKS = 1;    // Cadence de tir EXTRÊME (20 tirs par seconde !)
    private static final int RELOAD_TIME    = 45;   // 3 secondes pour recharger (un peu long pour un SMG)
       
    private static final float WEAPON_DAMAGE        = 8.0f;   // Dégâts très faibles par balle, compensés par la cadence
    private static final float PAP_BONUS_DAMAGE     = 12.0f;  // Excellent bonus PaP, la rend viable
       
    private static final float BASE_HEADSHOT_DAMAGE = 2.2f;   // Bon bonus de headshot
    private static final float PAP_HEADSHOT_BONUS   = 0.8f;   // Bonus supplémentaire au PaP
    
    // Paramètres de tir (projectile invisible)
    private static final float BASE_PROJECTILE_VELOCITY = 2.8f; // Vitesse des balles pour le projectile
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.15f; // Plus rapide après PaP
    private static final float BASE_PROJECTILE_SPREAD = 1.5f;   // Dispersion très élevée à la base
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.5f; // Très bonne réduction de dispersion avec PaP

    // Paramètres de Recul
    private static final float BASE_RECOIL_PITCH = 1.2f; // Recul vertical élevé
    private static final float BASE_RECOIL_YAW = 0.2f;   // Recul latéral notable
    private static final float PAP_RECOIL_MULTIPLIER = 0.6f; // Recul réduit avec PaP

    private static final SoundEvent FIRE_SOUND           = ZombieroolModSounds.THOMPSON_FIRE.get(); // Créer ce son
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); // Créer ce son
    private static final SoundEvent RELOAD_SOUND         = ZombieroolModSounds.THOMPSON_RELOADING.get(); // Créer ce son
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§5Pistolet-Tombe", "§5Pistolet-Tombe") // Translated PaP name - keep as is
            : getTranslatedMessage("§fThompson", "§fThompson"); // Thompson name likely same
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        ...
        tips.add(Component.literal(getTranslatedMessage("§aCadence de tir élevée, idéale pour les situations de mêlée.", "§aHigh rate of fire, ideal for close-quarters situations.")));
        if (upgraded) {
            tips.add(Component.literal(getTranslatedMessage("§dAméliorée via Pack-a-Punch", "§dUpgraded via Pack-a-Punch")));
            tips.add(Component.literal(getTranslatedMessage("§dNom PaP : Pistolet-Tombe", "§dPaP Name: Pistolet-Tombe"))); // Specific PaP name - keep as is
