...
public class SawWeaponItem extends Item implements IReloadable, ICustomWeapon, IPackAPunchable, IHeadshotWeapon {

    private static final int MAX_AMMO = 130; // Grande capacité de chargeur
    private static final int MAX_RESERVE = MAX_AMMO * 6; // Très grande réserve
    private static final int COOLDOWN_TICKS = 3; // Très bonne cadence de tir (0.15s entre les tirs)
    private static final int RELOAD_TIME = 95; // Rechargement lent (4.75s)
    
    private static final float WEAPON_DAMAGE = 10.5f; // Dégâts de base très élevés
    private static final float PAP_BONUS_DAMAGE = 16.0f; // Excellent bonus Pack-a-Punch
    
    private static final float BASE_HEADSHOT_DAMAGE = 2.0f; // Multiplicateur headshot solide
    private static final float PAP_HEADSHOT_BONUS = 12.0f; 
    // Dégâts de base du headshot : 10.5 * 2.0 = 21.0
    // Dégâts PaP du headshot : (10.5 + 16.0) * 2.0 + 12.0 = 26.5 * 2.0 + 12.0 = 53.0 + 12.0 = 65.0

    private static final float BASE_PROJECTILE_VELOCITY = 3.1f; // Bonne vitesse de projectile
    private static final float PAP_PROJECTILE_VELOCITY_MULTIPLIER = 1.12f; 
    private static final float BASE_PROJECTILE_SPREAD = 1.3f; // Dispersion modérée pour une LMG
    private static final float PAP_PROJECTILE_SPREAD_MULTIPLIER = 0.65f; // Très bonne amélioration de la précision PaP

    private static final float BASE_RECOIL_PITCH = 1.1f; // Recul vertical gérable mais présent
    private static final float BASE_RECOIL_YAW = 0.8f; // Recul horizontal modéré
    private static final float PAP_RECOIL_MULTIPLIER = 0.55f; // Très bonne amélioration du recul PaP

    private static final SoundEvent FIRE_SOUND = ZombieroolModSounds.SAW_FIRE.get(); // À définir
    private static final SoundEvent FIRE_SOUND_UPGRADED = ZombieroolModSounds.GUN_FIRE_UPGRADED.get(); 
    private static final SoundEvent RELOAD_SOUND = ZombieroolModSounds.SAW_RELOADING.get(); // À définir
    ...
    public SawWeaponItem() {
        super(new Properties().stacksTo(1).rarity(Rarity.EPIC)); // Rareté Épique
    }
    ...
    public Component getName(ItemStack stack) {
        ...
        String name = upgraded
            ? getTranslatedMessage("§5Chainbreaker", "§5Chainbreaker") // Translated PaP name
            : getTranslatedMessage("§6SAW", "§6SAW"); // Base name (proper noun, remains same)
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flag) {
        initializeIfNeeded(stack);
        boolean upgraded = isPackAPunched(stack);
        tips.add(Component.literal(getTranslatedMessage("§eLa mitrailleuse lourde standard des forces de l'UNSC.", "§eThe standard heavy machine gun of the UNSC forces.")));
        tips.add(Component.literal(getTranslatedMessage("§eExcellente pour les tirs de suppression et le contrôle de zone.", "§eExcellent for suppression fire and area control.")));
        if (upgraded) {
