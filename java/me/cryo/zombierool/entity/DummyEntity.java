...
public class DummyEntity extends Monster {
    ...
    public DummyEntity(EntityType<DummyEntity> type, Level world) {
        ...
        xpReward = 0;
        setNoAi(true);
        setCustomName(Component.translatable("entity.zombierool.dummy"));
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide && !damageAccumulatorMap.isEmpty()) {
            damageAccumulatorMap.forEach((uuid, acc) -> {
                ...
                message.append(Component.translatable("message.zombierool.dummy.took_damage"));
                message.append(Component.literal(String.format(" %.1f ", acc.totalDamage)).withStyle(ChatFormatting.RED));
                message.append(Component.translatable("message.zombierool.dummy.damage_hearts", String.format("%.1f", heartsLost)));
                if (acc.headshot) {
                    message.append(Component.translatable("message.zombierool.dummy.headshot").withStyle(ChatFormatting.GOLD));
                }
    ...
    public boolean hurt(DamageSource source, float amount) {
        if (!this.level().isClientSide) {
            ...
            boolean isTaczHeadshot = this.getPersistentData().getBoolean(DamageManager.HEADSHOT_TAG);
            final boolean finalIsHeadshot = isHeadshotCalc || isTaczHeadshot;
            this.getPersistentData().remove(DamageManager.HEADSHOT_TAG);
