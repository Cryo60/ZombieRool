...
public class PlasmaPistolItem extends WeaponSystem.BaseGunItem implements IHandgunWeapon, IOverheatable {
    ...
    public void onOverheatCooled(ItemStack stack, Player player) {
        playSound(player.level(), player, getDefinition().sounds.equip);
    }
    ...
    protected boolean executeShot(ItemStack stack, Player player, float charge, boolean isLeft) {
        ...
        int cooldownTicks = isOvercharged ? 30 : getDefinition().stats.fire_rate;

        if (player.hasEffect(ZombieroolModMobEffects.PERKS_EFFECT_DOUBLE_TAP.get())) {
    ...
    protected void performShooting(ItemStack stack, Player player, float charge, boolean isLeft) {
        ...
        float actualDamage = currentDef.stats.damage;

        if (isOvercharged) {
            float overchargeFactor = (charge - MIN_CHARGE_FOR_OVERCHARGE) / (1.0f - MIN_CHARGE_FOR_OVERCHARGE);
            actualDamage = currentDef.stats.damage * 1.5f + (currentDef.stats.damage * 1.5f * overchargeFactor);
        }
        if (isPap) {
            actualDamage += isOvercharged ? currentDef.pap.damage_bonus * 0.75f : currentDef.pap.damage_bonus;
        }

        Vec3 start = player.getEyePosition(1F);
        Vec3 dir   = player.getViewVector(1F);
        float velocity = isOvercharged ? currentDef.ballistics.velocity * 1.5f : currentDef.ballistics.velocity;
        float spread = getDynamicSpread(stack, player);

        if (isOvercharged) spread *= 0.125f;
        if (isPap) {
            velocity *= currentDef.pap.reload_speed_mult;
        }
        ...
        playSound(level, player, currentDef.sounds.fire);
    }
    ...
    public Component getName(ItemStack stack) {
        ...
        MutableComponent nameComponent = Component.literal((upgraded ? "§d" : "§2")).append(baseComp);
        ...
        public static void handleTick(Minecraft mc, Player player, ItemStack stack, boolean attackDown) {
            ...
            if (attackDown) {
                ...
                if (wasAttackDownLastTick) {
                    ...
                    NetworkHandler.INSTANCE.sendToServer(new C2SShootPacket(charge, false));
