...
public class ZRBlocks {
	...
	public static final RegistryObject<Block> STORAGE_BOX = registerBlock("storage_box", 
	    () -> new ZRDecorativeBlock(SoundType.WOOD, ZRDecorativeBlock.ShapeType.FULL, true));
	
	// Blocks without items (cannot be obtained in inventory)
	...
	private static <T extends Block> RegistryObject<T> registerBlockNoItem(String name, Supplier<T> blockFactory) {
	    // Registers the block but NOT the item, effectively removing it from inventory/creative tabs
	...
	private static void registerFamily(String name, SoundType sound, boolean transparent) {
	    ...
	    RegistryObject<Block> stairs = registerBlock(name + "_stairs", () -> new StairBlock(() -> baseBlock.get().defaultBlockState(),
